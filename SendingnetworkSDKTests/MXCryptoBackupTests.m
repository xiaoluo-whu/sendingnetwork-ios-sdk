...
- (void)testRESTBackupKeys
{
    [sendingnetworkSDKTestsData doMXRestClientTestWithAlice:self readyToTest:^(MXRestClient *aliceRestClient, XCTestExpectation *expectation) {
        ...
        [aliceRestClient createKeyBackupVersion:self.fakeKeyBackupVersion success:^(NSString *version) {
            ...
            NSString *roomId = @"!aRoomId:sending.network";
        ...
        [aliceSession.crypto.backup prepareKeyBackupVersionWithPassword:nil success:^(MXMegolmBackupCreationInfo * _Nonnull keyBackupCreationInfo) {
            [aliceSession.crypto.backup createKeyBackupVersion:keyBackupCreationInfo success:^(MXKeyBackupVersion * _Nonnull keyBackupVersion) {
                ...
                [aliceSession.crypto.backup trustForKeyBackupVersion:keyBackupVersion onComplete:^(MXKeyBackupVersionTrust * _Nonnull keyBackupVersionTrust) {
                    ...
                    XCTAssertEqualObjects(signature.deviceId, aliceSession.sendingnetworkRestClient.credentials.deviceId);
                    XCTAssertTrue(signature.valid);
                    XCTAssertEqualObjects(signature.device.deviceId, aliceSession.sendingnetworkRestClient.credentials.deviceId);
 ...
 */
- (void)testCheckAndStartKeyBackupWhenRestartingASendingnetworkSession
{
    // - Create a backup version
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoomWithCryptedMessages:self cryptedBob:YES readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
    ...
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoomWithCryptedMessages:self cryptedBob:YES readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [aliceSession.crypto.backup prepareKeyBackupVersionWithPassword:nil success:^(MXMegolmBackupCreationInfo * _Nonnull keyBackupCreationInfo) {
            [aliceSession.crypto.backup createKeyBackupVersion:keyBackupCreationInfo success:^(MXKeyBackupVersion * _Nonnull keyBackupVersion) {
                ...
                // - Create a new backup with fake data on the homeserver
                [aliceSession.sendingnetworkRestClient createKeyBackupVersion:self.fakeKeyBackupVersion success:^(NSString *version) {
        ...
        [aliceSession.crypto.backup prepareKeyBackupVersionWithPassword:nil success:^(MXMegolmBackupCreationInfo * _Nonnull keyBackupCreationInfo) {
            [aliceSession.crypto.backup createKeyBackupVersion:keyBackupCreationInfo success:^(MXKeyBackupVersion * _Nonnull keyBackupVersion) {
                [aliceSession.crypto.backup backupAllGroupSessions:^{
                    ...
                    NSString *oldDeviceId = aliceSession.sendingnetworkRestClient.credentials.deviceId;
...
- (void)testRestoreKeyBackupAndKeyShareRequests
{
    ...
    [self createKeyBackupScenarioWithPassword:nil readyToTest:^(NSString *version, MXMegolmBackupCreationInfo *keyBackupCreationInfo, NSArray<MXOlmInboundGroupSession *> *aliceKeys, MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        // - Check the SDK sent key share requests
        [sendingnetworkSDKTestsE2EData outgoingRoomKeyRequestInSession:aliceSession complete:^(MXOutgoingRoomKeyRequest *outgoingRoomKeyRequest) {
...
- (void)testGossipKey
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoomWithCryptedMessages:self cryptedBob:YES readyToTest:^(MXSession *aliceSession1, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
