...
@property (nonatomic, strong) SendingnetworkSDKTestsData *sendingnetworkSDKTestsData;
@property (nonatomic, strong) SendingnetworkSDKTestsE2EData *sendingnetworkSDKTestsE2EData;
...
- (void)setUp
{
    ...
    _sendingnetworkSDKTestsE2EData = [[SendingnetworkSDKTestsE2EData alloc] initWithSendingnetworkSDKTestsData:_sendingnetworkSDKTestsData];
...
- (void)tearDown
{
    ...
    _sendingnetworkSDKTestsE2EData = nil;
...
-(void)testDehydrateDevice
{
    // - Have e2e Alice
    MXWeakify(self);
    [self.sendingnetworkSDKTestsE2EData doE2ETestWithAliceInARoom:self readyToTest:^(MXSession *mxSession, NSString *roomId, XCTestExpectation *expectation) {
        MXStrongifyAndReturnIfNil(self);
        [mxSession.crypto.crossSigning setupWithPassword:MXTESTS_ALICE_PWD success:^{
            
            // - Alice creates a dehydrated device
            [self.dehydrationService dehydrateDeviceWithSendingnetworkRestClient:mxSession.sendingnetworkRestClient crypto:mxSession.crypto dehydrationKey:self.dehydrationKey success:^(NSString *dehydratedDeviceId) {
                // - Alice downloads their own devices keys
...
{
    // - Alice and Bob are in an e2e room
    [self.sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoom:self cryptedBob:YES warnOnUnknowDevices:NO readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        [bobSession.crypto.crossSigning setupWithPassword:MXTESTS_BOB_PWD success:^{
            
            NSString *bobUserId = bobSession.myUserId;
            
            // - Bob creates a dehydrated device and logs out
            [self.dehydrationService dehydrateDeviceWithSendingnetworkRestClient:bobSession.sendingnetworkRestClient crypto:bobSession.crypto dehydrationKey:self.dehydrationKey success:^(NSString *bobDehydratedDeviceId) {
...
-(void)testDeviceRehydrationWithoutDehydratedDevice
{
    ...
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithBob:self readyToTest:^(MXRestClient *bobRestClient, XCTestExpectation *expectation) {
        ...
        [self.sendingnetworkSDKTestsData retain:mxSession];

        // - Bob tries to rehydrate a device
        [self.dehydrationService rehydrateDeviceWithSendingnetworkRestClient:mxSession.sendingnetworkRestClient dehydrationKey:self.dehydrationKey success:^(NSString *deviceId) {
...
-(void)testDehydrateDeviceAndClaimDehydratedDevice
{
    ...
    [self.sendingnetworkSDKTestsE2EData doE2ETestWithAliceInARoom:self readyToTest:^(MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation) {
        [aliceSession.crypto.crossSigning setupWithPassword:MXTESTS_ALICE_PWD success:^{
            ...
            // - Alice setup a dehydrated device
            [self.dehydrationService dehydrateDeviceWithSendingnetworkRestClient:aliceSession.sendingnetworkRestClient crypto:aliceSession.crypto dehydrationKey:self.dehydrationKey success:^(NSString *dehydratedDeviceId) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    // - Alice logs off and logs in back
                    [self.sendingnetworkSDKTestsData loginUserOnANewDevice:self credentials:nil withPassword:MXTESTS_ALICE_PWD sessionToLogout:aliceSession newSessionStore:nil startNewSession:NO e2e:YES onComplete:^(MXSession *aliceSession2) {
                        
                        NSString *aliceSession2Device = aliceSession2.myDeviceId;
                        // - Alice rehydrate her device
                        [self.dehydrationService rehydrateDeviceWithSendingnetworkRestClient:aliceSession2.sendingnetworkRestClient dehydrationKey:self.dehydrationKey success:^(NSString *deviceId) {
                            // -> The rehydrated device must have the same properties
                            if (!deviceId)
                            {
...
-(void)testReceiveLiveMessageAfterDeviceRehydration
{
    // - Alice and Bob are in an e2e room
    [MXSDKOptions sharedInstance].enableCryptoWhenStartingMXSession = YES;
    [self.sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoomWithCryptedMessages:self cryptedBob:YES readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        [aliceSession.crypto.crossSigning setupWithPassword:MXTESTS_ALICE_PWD success:^{
            
            // - Alice creates a dehydrated device
            [self.dehydrationService dehydrateDeviceWithSendingnetworkRestClient:aliceSession.sendingnetworkRestClient crypto:aliceSession.crypto dehydrationKey:self.dehydrationKey success:^(NSString *deviceId) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    // - Alice logs out and logs on
                    [self.sendingnetworkSDKTestsData loginUserOnANewDevice:self credentials:nil withPassword:MXTESTS_ALICE_PWD sessionToLogout:aliceSession newSessionStore:nil startNewSession:NO e2e:YES onComplete:^(MXSession *aliceSession2) {
                        
                        MXRestClient *aliceRestClient = aliceSession2.sendingnetworkRestClient;
                        
                        MXSession *aliceSession3 = [[MXSession alloc] initWithSendingnetworkRestClient:aliceRestClient];
                        [self.sendingnetworkSDKTestsData retain:aliceSession3];
                        
                        [aliceSession2 close];
                        
                        // - Alice rehydrates the new session with the dehydrated device
                        [self.dehydrationService rehydrateDeviceWithSendingnetworkRestClient:aliceSession3.sendingnetworkRestClient dehydrationKey:self.dehydrationKey success:^(NSString *rehydratedDeviceId) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [bobSession logout:^{
                        ...
                        [roomFromAlicePOV sendTextMessage:message threadId:nil success:^(NSString *eventId) {
                            ...
                            // - Bob logs in on a new device
                            [self.sendingnetworkSDKTestsData loginUserOnANewDevice:self credentials:bobCredentials withPassword:MXTESTS_BOB_PWD sessionToLogout:nil newSessionStore:nil startNewSession:NO e2e:YES onComplete:^(MXSession *bobSession2) {
                                
                                // - Bob rehydrates the new session with the dehydrated device
                                [self.dehydrationService rehydrateDeviceWithSendingnetworkRestClient:bobSession2.sendingnetworkRestClient dehydrationKey:self.dehydrationKey success:^(NSString *deviceId) {
                                    if (!deviceId)
                                    {
                                        XCTFail(@"device rehydration shouldn't be canceled");
                                        [expectation fulfill];
