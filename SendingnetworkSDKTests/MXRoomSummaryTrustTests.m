...
{
    // - Alice and Bob are in a room
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoom:self cryptedBob:YES warnOnUnknowDevices:YES readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
...
{
    // - Have Alice with 2 devices (Alice1 and Alice2) and Bob. All trusted via cross-signing
    [sendingnetworkSDKTestsE2EData doTestWithBobAndAliceWithTwoDevicesAllTrusted:self readyToTest:^(MXSession *aliceSession1, MXSession *aliceSession2, MXSession *bobSession1, NSString *roomId, XCTestExpectation *expectation) {
        
        [sendingnetworkSDKTestsE2EData loginUserOnANewDevice:self credentials:bobSession1.sendingnetworkRestClient.credentials withPassword:MXTESTS_BOB_PWD onComplete:^(MXSession *bobSession2) {
