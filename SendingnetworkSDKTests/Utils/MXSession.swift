...
/// An extension for reporting calls to `init(sendingnetworkRestClient:)` and `close()` on `MXSession` instances
extension MXSession {
    ...
    class func trackOpenMXSessions() {
        ...
        swizzleMethods(originalSelector: #selector(MXSession.init(sendingnetworkRestClient:)),
                       swizzledSelector: #selector(MXSession.trackInit(sendingnetworkRestClient:)))
    ...
    /// Swizzled version of MXSession.init(sendingnetworkRestClient:)
    @objc
    private func trackInit(sendingnetworkRestClient: MXRestClient) -> MXSession {
        // Call the original method. Note that implementations are exchanged
        _ = self.trackInit(sendingnetworkRestClient: sendingnetworkRestClient)
