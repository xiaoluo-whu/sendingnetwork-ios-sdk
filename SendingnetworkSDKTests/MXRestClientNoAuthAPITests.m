...
- (void)testSupportedSendingnetworkVersions
{
    XCTestExpectation *expectation = [self expectationWithDescription:@"asyncTest"];

    [mxRestClient supportedSendingnetworkVersions:^(MXSendingnetworkVersions *sendingnetworkVersions) {

        XCTAssertNotNil(sendingnetworkVersions);
        XCTAssertNotNil(sendingnetworkVersions.versions);

        // Check supported spec version at the time of writing this test
        XCTAssert([sendingnetworkVersions.versions containsObject:@"r0.0.1"]);
        XCTAssert([sendingnetworkVersions.versions containsObject:@"r0.1.0"]);
        XCTAssert([sendingnetworkVersions.versions containsObject:@"r0.2.0"]);
        XCTAssert([sendingnetworkVersions.versions containsObject:@"r0.3.0"]);
...
// At the time of introducing this test, MXSendingnetworkVersions.supportLazyLoadMembers
// was stored in MXSendingnetworkVersions.unstableFeatures.
// Make sure that, in future versions of the spec, supportLazyLoadMembers is still YES
// wherever it will be stored.
- (void)testSupportedSendingnetworkVersionsSupportLazyLoadMembers
{
    XCTestExpectation *expectation = [self expectationWithDescription:@"asyncTest"];

    [mxRestClient supportedSendingnetworkVersions:^(MXSendingnetworkVersions *sendingnetworkVersions) {

        XCTAssert(sendingnetworkVersions.supportLazyLoadMembers);
...
{
    [sendingnetworkSDKTestsData doMXRestClientTestWithBobAndThePublicRoom:self readyToTest:^(MXRestClient *bobRestClient, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [bobRestClient publicRoomsOnServer:nil limit:-1 since:nil filter:nil thirdPartyInstanceId:nil includeAllNetworks:NO success:^(MXPublicRoomsResponse *publicRoomsResponse) {
            ...
            XCTAssertGreaterThan(theMXPublicRoom.numJoinedMembers, 0, @"The is at least mxBob at #sendingnetwork:sending.network");
