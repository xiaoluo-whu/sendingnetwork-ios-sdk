...
- (void)testEditServerSide
{
    ...
    [self createScenario:^(MXSession *mxSession, MXRoom *room, XCTestExpectation *expectation, NSString *eventId, NSString *editEventId) {
        ...
        // -> Check data is correctly aggregated when fetching the edited event directly from the homeserver
        [mxSession.sendingnetworkRestClient eventWithEventId:eventId inRoom:room.roomId success:^(MXEvent *event) {
...
- (void)testEditsFromInitialSync
{
    ...
    [self createScenario:^(MXSession *mxSession, MXRoom *room, XCTestExpectation *expectation, NSString *eventId, NSString *editEventId) {
        ...
        MXRestClient *restClient = mxSession.sendingnetworkRestClient;
        ...
        // - Do an initial sync
        mxSession = [[MXSession alloc] initWithSendingnetworkRestClient:restClient];
...
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoom:self cryptedBob:YES warnOnUnknowDevices:NO aliceStore:[[MXMemoryStore alloc] init] bobStore:[[MXMemoryStore alloc] init] readyToTest:^(MXSession *mxSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
...
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoom:self cryptedBob:YES warnOnUnknowDevices:NO aliceStore:[[MXMemoryStore alloc] init] bobStore:[[MXMemoryStore alloc] init] readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
