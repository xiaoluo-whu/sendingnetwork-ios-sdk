...
class MXBackgroundSyncServiceTests: XCTestCase {
    ...
    private var e2eTestData: SendingnetworkSDKTestsE2EData!
    ...
    override func setUp() {
        ...
        e2eTestData = SendingnetworkSDKTestsE2EData(sendingnetworkSDKTestsData: testData)
    }
                ...
                case .success(let eventId):
                    ...
                    self.bgSyncService?.event(withEventId: eventId, inRoom: roomId) { (response) in
                        ...
                        case .success(let event):
                            ...
                            // - Bob restarts their MXSession
                            let newBobSession = MXSession(sendingnetworkRestClient: MXRestClient(credentials: bobCredentials, unrecognizedCertificateHandler: nil))
    ...
    func testWithEncryptedEventRollingKeys() {
        ...
        e2eTestData.doE2ETestWithAliceAndBob(inARoom: self, cryptedBob: true, warnOnUnknowDevices: warnOnUnknownDevices, aliceStore: aliceStore, bobStore: bobStore) { (aliceSession, bobSession, roomId, expectation) in
            ...
            guard let aliceRestClient = aliceSession?.sendingnetworkRestClient, let bobCredentials = bobSession?.credentials else {
                XCTFail("Cannot set up initial test conditions - error: Bob's credentials cannot be retrieved")
                expectation?.fulfill()
                return
            }
    ...
    func testMXSessionDecryptionAfterClearCache() {
        ...
        self.createStoreScenario(messageCountChunks: [5, Constants.numberOfMessagesForLimitedTest, 1]) { (aliceSession, bobSession, bobBgSyncService, roomId, eventIdsChunks, expectation) in
            ...
            self.addToDeviceEventToStore(of: bobBgSyncService, otherSession: aliceSession, roomId: roomId) { _ in
                ...
                guard let bobSession2 = MXSession(sendingnetworkRestClient: restClient) else {
