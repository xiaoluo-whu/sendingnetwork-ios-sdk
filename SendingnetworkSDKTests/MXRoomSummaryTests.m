...
- (void)testGetLastMessageFromPagination
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndARoomWithMessages:self readyToTest:^(MXSession *mxSession, MXRoom *room, XCTestExpectation *expectation) {
        ...
        MXRestClient *bobRestClient = mxSession.sendingnetworkRestClient;
...
- (void)testGetLastMessageFromSeveralPaginations
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndARoomWithMessages:self readyToTest:^(MXSession *mxSession, MXRoom *room, XCTestExpectation *expectation) {
        ...
        [self->sendingnetworkSDKTestsData for:bobRestClient andRoom:roomId sendMessages:80 testCase:self success:^{
...
- (void)testRoomAvatarExcludingUsers
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndAliceInARoom:self readyToTest:^(MXSession *bobSession, MXRestClient *aliceRestClient, NSString *roomId, XCTestExpectation *expectation) {
        ...
        NSString *avatarURL = @"http://sending.network/sendingnetwork.png";
...
- (void)testMembership
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndARoom:self andStore:[[MXFileStore alloc] init] readyToTest:^(MXSession *mxSession, MXRoom *room, XCTestExpectation *expectation) {
...
- (void)testInvite
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndAliceInARoom:self readyToTest:^(MXSession *bobSession, MXRestClient *aliceRestClient, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [aliceRestClient createRoom:newRoomName visibility:kMXRoomDirectoryVisibilityPrivate roomAlias:nil topic:newRoomTopic success:^(MXCreateRoomResponse *response) {
            ...
            [aliceRestClient inviteUser:bobSession.sendingnetworkRestClient.credentials.userId toRoom:newRoomId success:nil failure:^(NSError *error) {
                XCTFail(@"Cannot set up intial test conditions - error: %@", error);
                [expectation fulfill];
            }];
...
{
    // Test it on a permanent store
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceInARoom:self andStore:[[MXFileStore alloc] init] readyToTest:^(MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation) {
...
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoom:self
                                                cryptedBob:YES
...
// https://github.com/sending-network/sendingnetwork-ios-sdk/issues/409
...
{
    // 1 - Bob creates a room and invite Alice
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndAliceInARoom:self andStore:[[MXFileStore alloc] init] readyToTest:^(MXSession *bobSession, MXRestClient *aliceRestClient, NSString *roomId, XCTestExpectation *expectation) {
