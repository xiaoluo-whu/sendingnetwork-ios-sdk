...
- (void)testMXSessionMyUser
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndARoomWithMessages:self readyToTest:^(MXSession *mxSession, MXRoom *room, XCTestExpectation *expectation) {
        ...
        MXUser *myUser = [mxSession userWithUserId:mxSession.sendingnetworkRestClient.credentials.userId];
    ...
    [sendingnetworkSDKTestsData doMXRestClientTestWithAlice:self readyToTest:^(MXRestClient *aliceRestClient, XCTestExpectation *expectation) {
        ...
        [mxSession start:^{
            ...
            [mxSession.myUser listenToUserUpdate:^(MXEvent *event) {
                ...
                XCTAssertEqual(event.eventType, MXEventTypePresence);
                XCTAssert([mxSession.myUser.avatarUrl isEqualToString:@"mxc://sending.network/rQkrOoaFIRgiACATXUdQIuNJ"], @"Wrong avatar. Found: %@", mxSession.myUser.avatarUrl);

            }];

            // Update the profile with a mxc URL (non mxc url are ignored)
            [mxSession.myUser setAvatarUrl:@"mxc://sending.network/rQkrOoaFIRgiACATXUdQIuNJ" success:^{
...
- (void)testIdenticon
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndARoomWithMessages:self readyToTest:^(MXSession *mxSession, MXRoom *room, XCTestExpectation *expectation) {
        ...
        XCTAssert([identiconURL hasPrefix:@"http://localhost:8080/_api/media/v1/identicon/%40mxbob"]);
