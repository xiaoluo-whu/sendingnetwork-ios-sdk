...
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndAliceInARoom:self readyToTest:^(MXSession *bobSession, MXRestClient *aliceRestClient, NSString *aRoomId, XCTestExpectation *expectation) {
...
- (void)testDirectRoomsAfterInitialSync
{
    [self createScenario:^(MXSession *bobSession, NSString *aliceUserId, NSString *charlieUserId, XCTestExpectation *expectation) {
        ...
        // - Bob does an initial /sync
        MXSession *bobSession2 = [[MXSession alloc] initWithSendingnetworkRestClient:bobSession.sendingnetworkRestClient];
        [sendingnetworkSDKTestsData retain:bobSession2];
        [bobSession close];
...
- (void)testDirectRoomsRaceConditions
{
    [self createScenario:^(MXSession *bobSession, NSString *aliceUserId, NSString *charlieUserId, XCTestExpectation *expectation) {
        ...
        [roomWithCharlie setIsDirect:YES withUserId:@"@aRandomUser:sending.network" success:^{
            successCount++;
            XCTAssertEqualObjects(roomWithCharlie.directUserId, @"@aRandomUser:sending.network");
        } failure:^(NSError *error) {
            XCTFail(@"The operation should not fail - NSError: %@", error);
            [expectation fulfill];
        }];
...
- (void)testSummaryStorage
{
    ...
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndAliceInARoom:self andStore:store readyToTest:^(MXSession *bobSession, MXRestClient *aliceRestClient, NSString *roomId, XCTestExpectation *expectation) {
