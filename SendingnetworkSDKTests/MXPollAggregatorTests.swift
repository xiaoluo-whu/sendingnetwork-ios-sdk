...
class MXPollAggregatorTest: XCTestCase {
    private var sendingnetworkSDKTestsData: SendingnetworkSDKTestsData!
        
    private var sendingnetworkSDKTestsE2EData: SendingnetworkSDKTestsE2EData!

    private var pollAggregator: PollAggregator!
    
    override func setUp() {
        sendingnetworkSDKTestsData = SendingnetworkSDKTestsData()
        sendingnetworkSDKTestsE2EData = SendingnetworkSDKTestsE2EData(sendingnetworkSDKTestsData: sendingnetworkSDKTestsData)
    }
    ...
    func testGappySync() {
        self.createScenarioForBobAndAlice { bobSession, aliceSession, bobRoom, aliceRoom, pollStartEvent, expectation in
            ...
            self.sendingnetworkSDKTestsData.for(bobSession.sendingnetworkRestClient, andRoom: bobRoom.roomId, sendMessages: 50, testCase: self) {
                bobRoom.sendPollResponse(for: pollStartEvent, withAnswerIdentifiers: ["1"], threadId:nil, localEcho: nil) { _ in
                    aliceRoom.sendPollResponse(for: pollStartEvent, withAnswerIdentifiers: ["1", "2"], threadId:nil, localEcho: nil) { _ in
                        self.sendingnetworkSDKTestsData.for(aliceSession.sendingnetworkRestClient, andRoom: aliceRoom.roomId, sendMessages: 50, testCase: self) {
    ...
    func createScenarioForBobAndAlice(_ readyToTest: @escaping (MXSession, MXSession, MXRoom, MXRoom, MXEvent, XCTestExpectation) -> Void) {
        ...
        self.sendingnetworkSDKTestsData.doTestWithAliceAndBob(inARoom: self, aliceStore: MXMemoryStore(), bobStore: MXMemoryStore()) { aliceSession, bobSession, roomId, expectation in
