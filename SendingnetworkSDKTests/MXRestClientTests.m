...
@property (nonatomic, strong, readonly) SendingnetworkSDKTestsData *sendingnetworkSDKTestsData;
...
- (void)setUp
{
    ...
    _sendingnetworkSDKTestsData = [[SendingnetworkSDKTestsData alloc] init];
}
...
- (void)tearDown
{
    ...
    _sendingnetworkSDKTestsData = nil;
}

- (void)testInit
{
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithBob:self readyToTest:^(MXRestClient *bobRestClient, XCTestExpectation *expectation) {
        ...
        XCTAssertTrue([bobRestClient.credentials.userId isEqualToString:self.sendingnetworkSDKTestsData.bobCredentials.userId], "bobRestClient.userId(%@) is wrong", bobRestClient.credentials.userId);
        XCTAssertTrue([bobRestClient.credentials.accessToken isEqualToString:self.sendingnetworkSDKTestsData.bobCredentials.accessToken], "bobRestClient.accessToken(%@) is wrong", bobRestClient.credentials.accessToken);
...
- (void)testClose
{
    ...
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithBobAndARoom:self readyToTest:^(MXRestClient *bobRestClient, NSString *roomId, XCTestExpectation *expectation) {
...
- (void)testRoomAvatar
{
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithBobAndARoom:self readyToTest:^(MXRestClient *bobRestClient, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [bobRestClient setRoomAvatar:roomId avatar:@"http://sending.network/sendingnetwork.png" success:^{

            [bobRestClient2 avatarOfRoom:roomId success:^(NSString *avatar) {

                XCTAssertNotNil(avatar);
                XCTAssertNotEqual(avatar.length, 0);
                XCTAssertEqualObjects(avatar, @"http://sending.network/sendingnetwork.png");
                [expectation fulfill];
...
{
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithBobAndThePublicRoom:self readyToTest:^(MXRestClient *bobRestClient, NSString *roomId, XCTestExpectation *expectation) {

        [bobRestClient joinRoom:self.sendingnetworkSDKTestsData.thePublicRoomAlias viaServers:nil withThirdPartySigned:nil success:^(NSString *theRoomId) {
...
- (void)testInviteUserToRoom
{
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithBobAndARoom:self readyToTest:^(MXRestClient *bobRestClient, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [self.sendingnetworkSDKTestsData doMXRestClientTestWithAlice:nil readyToTest:^(MXRestClient *aliceRestClient, XCTestExpectation *expectation2) {
            
            // Do the test
            [bobRestClient inviteUser:self.sendingnetworkSDKTestsData.aliceCredentials.userId toRoom:roomId success:^{
                ...
                [bobRestClient membersOfRoom:roomId success:^(NSArray *roomMemberEvents) {
                    ...
                    for (MXEvent *roomMemberEvent in roomMemberEvents)
                    {
                        ...
                        if ([member.userId isEqualToString:self.sendingnetworkSDKTestsData.aliceCredentials.userId])
                        {
                        ...
                        else
                        {
                            ...
                            XCTAssert([member.userId isEqualToString:self.sendingnetworkSDKTestsData.bobCredentials.userId], @"Unexpected member: %@", member);
                        }
                    }
...
{
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithBobAndAliceInARoom:self readyToTest:^(MXRestClient *bobRestClient, MXRestClient *aliceRestClient, NSString *roomId, XCTestExpectation *expectation) {
        
        [bobRestClient kickUser:self.sendingnetworkSDKTestsData.aliceCredentials.userId fromRoom:roomId reason:@"No particular reason" success:^{
...
- (void)testBanUserInRoom
{
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithBobAndAliceInARoom:self readyToTest:^(MXRestClient *bobRestClient, MXRestClient *aliceRestClient, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [bobRestClient banUser:self.sendingnetworkSDKTestsData.aliceCredentials.userId inRoom:roomId reason:@"No particular reason" success:^{
...
- (void)testCreateRoomWithInvite
{
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithBob:self readyToTest:^(MXRestClient *bobRestClient, XCTestExpectation *expectation) {
        ...
        [self.sendingnetworkSDKTestsData doMXRestClientTestWithAlice:nil readyToTest:^(MXRestClient *aliceRestClient, XCTestExpectation *expectation2) {
            ...
            parameters.inviteArray = @[self.sendingnetworkSDKTestsData.aliceCredentials.userId];
            [bobRestClient createRoomWithParameters:parameters success:^(MXCreateRoomResponse *response) {
                ...
                [bobRestClient membersOfRoom:response.roomId success:^(NSArray *roomMemberEvents) {
                    ...
                    {
                        succeed = [roomMemberEvent2.stateKey isEqualToString:self.sendingnetworkSDKTestsData.aliceCredentials.userId];
                    }
                    else if ([roomMemberEvent1.stateKey isEqualToString:self.sendingnetworkSDKTestsData.aliceCredentials.userId])
                    {
...
{
    [self.sendingnetworkSDKTestsData  doMXRestClientTestWithBobAndARoomWithMessages:self readyToTest:^(MXRestClient *bobRestClient, NSString *roomId, XCTestExpectation *expectation) {
...
- (void)testFilter
{
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithAlice:self readyToTest:^(MXRestClient *aliceRestClient, XCTestExpectation *expectation) {
        ...
        filter.room.rooms = @[@"!aroom:sendingnetwork:org"];
        filter.room.notRooms = @[@"!notaroom:sendingnetwork:org"];
        ...
        filter.room.ephemeral.rooms = @[@"!aroom_ephemeral:sendingnetwork:org"];
        filter.room.ephemeral.notRooms = @[@"!notaroom_ephemeral:sendingnetwork:org"];
        filter.room.ephemeral.senders = @[@"@asender:sending.network"];
        filter.room.ephemeral.notSenders = @[@"@notasender:sending.network"];
        ...
        filter.presence.senders = @[@"@asender:sending.network"];
        filter.presence.notSenders = @[@"@notasender:sending.network"];
...
- (void)testOtherUserDisplayName
{
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithAlice:self readyToTest:^(MXRestClient *aliceRestClient, XCTestExpectation *expectation) {
        ...
        [aliceRestClient setDisplayName:newDisplayName success:^{
            ...
            [self.sendingnetworkSDKTestsData doMXRestClientTestWithBob:nil readyToTest:^(MXRestClient *bobRestClient, XCTestExpectation *expectation2) {
                
                // Then retrieve it from a Bob restClient
                [bobRestClient displayNameForUser:self.sendingnetworkSDKTestsData.aliceCredentials.userId success:^(NSString *displayname) {
...
- (void)testUserAvatarUrl
{
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithAlice:self readyToTest:^(MXRestClient *aliceRestClient, XCTestExpectation *expectation) {
        ...
        // Set the avatar url
        __block NSString *newAvatarUrl = @"http://sending.network/sendingnetwork2.png";
    ...
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithAlice:self readyToTest:^(MXRestClient *aliceRestClient, XCTestExpectation *expectation) {
        ...
        [aliceRestClient setAvatarUrl:newAvatarUrl success:^{
            ...
            [self.sendingnetworkSDKTestsData doMXRestClientTestWithBob:nil readyToTest:^(MXRestClient *bobRestClient, XCTestExpectation *expectation2) {
                ...
                // Then retrieve it from a Bob restClient
                [bobRestClient avatarUrlForUser:self.sendingnetworkSDKTestsData.aliceCredentials.userId success:^(NSString *avatarUrl) {
...
{
    [self.sendingnetworkSDKTestsData doMXSessionTestWithBobAndARoomWithMessages:self readyToTest:^(MXSession *mxSession, MXRoom *room, XCTestExpectation *expectation) {
        ...
        [mxSession listenToEventsOfTypes:@[kMXEventTypeStringRoomMessage] onEvent:^(MXEvent *event, MXTimelineDirection direction, id customObject) {
            ...
            [mxSession.sendingnetworkRestClient searchMessagesWithText:message
                                               roomEventFilter:nil
                                                   beforeLimit:3
                                                    afterLimit:1
                                                     nextBatch:nil
...
- (void)testSearchPaginate
{
    [self.sendingnetworkSDKTestsData doMXRestClientTestWithBobAndARoom:self readyToTest:^(MXRestClient *bobRestClient, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [self.sendingnetworkSDKTestsData for:bobRestClient andRoom:roomId sendMessages:20 testCase:self success:^{
