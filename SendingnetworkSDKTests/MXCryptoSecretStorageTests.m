...
NSString *jsSDKDataPassphrase = @"ILoveSendingnetwork&Riot";
...
// Have Alice with SSSS bootstrapped with data built by sendingnetwork-js-sdk
- (void)createScenarioWithSendingnetworkJsSDKData:(void (^)(MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation))readyToTest
{
    ...
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceInARoom:self readyToTest:^(MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        // Feed the session with data built with sendingnetwork-js-sdk (extracted from Riot)
...
{
    // - Have Alice with SSSS bootstrapped
    [self createScenarioWithSendingnetworkJsSDKData:^(MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation) {
...
// -> We should get it
// -> It should be the one created by sendingnetwork-js-sdk
- (void)testGetSecret
{
    // - Have Alice with SSSS bootstrapped
    [self createScenarioWithSendingnetworkJsSDKData:^(MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation) {
