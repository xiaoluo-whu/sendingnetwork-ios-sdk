// 
// Copyright 2022 The Sending.network Foundation C.I.C
//
...
import Foundation
@testable import SendingnetworkSDK
...
class MXMegolmEncryptionTests: XCTestCase {
    ...
    private var data: SendingnetworkSDKTestsData!
    private var e2eData: SendingnetworkSDKTestsE2EData!
    override func setUp() {
        super.setUp()
        
        data = SendingnetworkSDKTestsData()
        e2eData = SendingnetworkSDKTestsE2EData(sendingnetworkSDKTestsData: data)
    }
    
    private func storedSessionIds(in session: MXSession!) -> SessionIds {
        let store = MXRealmCryptoStore(credentials: session.sendingnetworkRestClient!.credentials)
        return SessionIds(
