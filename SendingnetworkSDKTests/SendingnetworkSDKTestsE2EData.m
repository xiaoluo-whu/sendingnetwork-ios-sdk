...
@interface SendingnetworkSDKTestsE2EData ()

@property (nonatomic, weak) SendingnetworkSDKTestsData *sendingnetworkSDKTestsData;

@end

@implementation SendingnetworkSDKTestsE2EData
@synthesize sendingnetworkSDKTestsData, messagesFromAlice, messagesFromBob;

- (instancetype)initWithSendingnetworkSDKTestsData:(SendingnetworkSDKTestsData *)theSendingnetworkSDKTestsData
{
    self = [super init];
    if (self)
    {
        sendingnetworkSDKTestsData = theSendingnetworkSDKTestsData;
                     ...
                     readyToTest:(void (^)(MXSession *bobSession, MXSession *aliceSession, XCTestExpectation *expectation))readyToTest
{
    ...
    [sendingnetworkSDKTestsData doMXSessionTestWithBob:testCase readyToTest:^(MXSession *bobSession, XCTestExpectation *expectation) {

        [sendingnetworkSDKTestsData doMXSessionTestWithAlice:nil readyToTest:^(MXSession *aliceSession, XCTestExpectation *expectation2) {
...
- (void)doE2ETestWithAliceInARoom:(XCTestCase *)testCase andStore:(id<MXStore>)store readyToTest:(void (^)(MXSession *, NSString *, XCTestExpectation *))readyToTest
{
    ...
    [sendingnetworkSDKTestsData doMXSessionTestWithAlice:testCase andStore:store
        ...
        [aliceSession createRoom:nil visibility:kMXRoomDirectoryVisibilityPrivate roomAlias:nil topic:nil success:^(MXRoom *room) {
            ...
            [room enableEncryptionWithAlgorithm:kMXCryptoMegolmAlgorithm success:^{
                ...
                [sendingnetworkSDKTestsData breakTestCase:testCase reason:@"Cannot enable encryption in room - error: %@", error];
            }];

        } failure:^(NSError *error) {
            [sendingnetworkSDKTestsData breakTestCase:testCase reason:@"Cannot create a room - error: %@", error];
        }];
                            ...
                            readyToTest:(void (^)(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation))readyToTest
{
    [self doE2ETestWithAliceInARoom:testCase andStore:aliceStore readyToTest:^(MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [sendingnetworkSDKTestsData doMXSessionTestWithBob:nil andStore:bobStore readyToTest:^(MXSession *bobSession, XCTestExpectation *expectation2) {
            ...
            __block __weak id observer = [[NSNotificationCenter defaultCenter] addObserverForName:kMXSessionNewRoomNotification object:bobSession queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *note) {
                ...
                [bobSession joinRoom:note.userInfo[kMXSessionNotificationRoomIdKey] viaServers:nil success:^(MXRoom *room) {
                    ...
                    [sendingnetworkSDKTestsData breakTestCase:testCase reason:@"Cannot join a room - error: %@", error];
                }];
            }];

            [room inviteUser:bobSession.myUser.userId success:nil failure:^(NSError *error) {
                [[NSNotificationCenter defaultCenter] removeObserver:observer];
                [sendingnetworkSDKTestsData breakTestCase:testCase reason:@"Cannot invite Bob (%@) - error: %@", bobSession.myUser.userId, error];
            }];
                                               ...
                                               readyToTest:(void (^)(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation))readyToTest
{
    [self doE2ETestWithAliceAndBobInARoom:testCase cryptedBob:cryptedBob warnOnUnknowDevices:NO readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [roomFromAlicePOV sendTextMessage:messagesFromAlice[0] threadId:nil success:^(NSString *eventId) {
            ...
            [sendingnetworkSDKTestsData breakTestCase:testCase reason:@"Cannot set up intial test conditions - error: %@", error];
        }];
                                  ...
                                  readyToTest:(void (^)(MXSession *aliceSession, MXSession *bobSession, MXSession *samSession, NSString *roomId, XCTestExpectation *expectation))readyToTest
{
    [self doE2ETestWithAliceAndBobInARoom:testCase cryptedBob:cryptedBob warnOnUnknowDevices:warnOnUnknowDevices readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        // Ugly hack: Create a bob from another SendingnetworkSDKTestsData instance and call him Sam...
        SendingnetworkSDKTestsData *sendingnetworkSDKTestsData2 = [[SendingnetworkSDKTestsData alloc] init];
        [sendingnetworkSDKTestsData2 doMXSessionTestWithBob:nil readyToTest:^(MXSession *samSession, XCTestExpectation *expectation2) {
            ...
            [room inviteUser:samSession.myUser.userId success:nil failure:^(NSError *error) {
                [[NSNotificationCenter defaultCenter] removeObserver:observer];
                [sendingnetworkSDKTestsData breakTestCase:testCase reason:@"Cannot invite Alice - error: %@", error];
            }];
                   ...
                   onComplete:(void (^)(MXSession *newSession))onComplete
{
    ...
    [sendingnetworkSDKTestsData retain:mxRestClient];
    ...
    [mxRestClient loginWithLoginType:kMXLoginFlowTypePassword username:credentials.userId password:password success:^(MXCredentials *credentials2) {
        ...
        [sendingnetworkSDKTestsData retain:mxRestClient2];
        
        MXSession *newSession = [[MXSession alloc] initWithSendingnetworkRestClient:mxRestClient2];
        [sendingnetworkSDKTestsData retain:newSession];
        ...
        [newSession setStore:store success:^{
            ...
            [sendingnetworkSDKTestsData breakTestCase:testCase reason:@"Cannot set up store - error: %@", error];
        }];
        
    } failure:^(NSError *error) {
        [sendingnetworkSDKTestsData breakTestCase:testCase reason:@"Cannot log %@ in again. Error: %@", credentials.userId , error];
    }];
}
                                               ...
                                               readyToTest:^(MXSession *aliceSession1, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation)
     {
         ...
         [aliceSession1.crypto.crossSigning setupWithPassword:MXTESTS_ALICE_PWD success:^{
             [bobSession.crypto.crossSigning setupWithPassword:MXTESTS_BOB_PWD success:^{
                 ...
                 [self loginUserOnANewDevice:testCase credentials:aliceSession1.sendingnetworkRestClient.credentials withPassword:MXTESTS_ALICE_PWD onComplete:^(MXSession *aliceSession2) {
