...
{
    // - Create Alice & Bob account
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoom:testCase
                                                cryptedBob:YES
                  ...
                  readyToTest:(void (^)(MXSession *bobSession, MXSession *alice1Session, NSString *roomId, MXCredentials *alice0Creds, XCTestExpectation *expectation))readyToTest
{
    ...
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoom:testCase
     ...
     {
        MXCredentials *alice0Creds = alice0Session.sendingnetworkRestClient.credentials;

        // - Log Alice on a new device
        [MXSDKOptions sharedInstance].enableCryptoWhenStartingMXSession = YES;
        [self->sendingnetworkSDKTestsData relogUserSessionWithNewDevice:self session:alice0Session withPassword:MXTESTS_ALICE_PWD onComplete:^(MXSession *alice1Session) {
...
- (void)testBootstrapWithPassword
{
    ...
    [sendingnetworkSDKTestsE2EData doE2ETestWithBobAndAlice:self readyToTest:^(MXSession *bobSession, MXSession *aliceSession, XCTestExpectation *expectation) {
        ...
        [aliceSession.crypto.crossSigning setupWithPassword:MXTESTS_ALICE_PWD success:^{
            ...
            // -> Alice must see their device trusted
            MXDeviceTrustLevel *aliceDevice1Trust = [aliceSession.crypto deviceTrustLevelForDevice:aliceSession.sendingnetworkRestClient.credentials.deviceId ofUser:aliceSession.sendingnetworkRestClient.credentials.userId];
        ...
        [aliceSession.crypto.crossSigning setupWithPassword:MXTESTS_ALICE_PWD success:^{
            ...
            [sendingnetworkSDKTestsE2EData loginUserOnANewDevice:self credentials:aliceSession.sendingnetworkRestClient.credentials withPassword:MXTESTS_ALICE_PWD onComplete:^(MXSession *newAliceSession) {
                ...
                [newAliceSession.crypto.crossSigning refreshStateWithSuccess:^(BOOL stateUpdated) {
                    ...
                    // - Make each device trust each other
                    //   This simulates a self verification and trigger cross-signing behind the shell
                    [newAliceSession.crypto setDeviceVerification:MXDeviceVerified forDevice:aliceSession.sendingnetworkRestClient.credentials.deviceId ofUser:aliceSession.sendingnetworkRestClient.credentials.userId success:^{
                        
                        [aliceSession.crypto setDeviceVerification:MXDeviceVerified forDevice:newAliceSession.sendingnetworkRestClient.credentials.deviceId ofUser:aliceSession.sendingnetworkRestClient.credentials.userId success:^{
...
- (void)testKeysDownloadCSAPIs
{
    ...
    [self doTestWithBobAndBootstrappedAlice:self readyToTest:^(MXSession *bobSession, MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        NSString *aliceUserId = aliceSession.sendingnetworkRestClient.credentials.userId;
        NSString *aliceDeviceId = aliceSession.sendingnetworkRestClient.credentials.deviceId;

        // - Use the CS API to retrieve alice keys
        [aliceSession.sendingnetworkRestClient downloadKeysForUsers:@[aliceUserId] token:nil success:^(MXKeysQueryResponse *keysQueryResponse) {
...
- (void)testSignUser2
{
    ...
    [self doBootstrappedTestBobAndAliceWithTwoDevices:self readyToTest:^(MXSession *bobSession, MXSession *alice1Session, NSString *roomId, MXCredentials *alice0Creds, XCTestExpectation *expectation) {
        ...
        [bobSession.crypto.crossSigning signUserWithUserId:alice0Creds.userId success:^{
            ...
            MXDeviceTrustLevel *aliceDevice1Trust = [bobSession.crypto deviceTrustLevelForDevice:alice1Session.sendingnetworkRestClient.credentials.deviceId ofUser:alice0Creds.userId];
            XCTAssertNotNil(aliceDevice1Trust);
            XCTAssertTrue(aliceDevice1Trust.isCrossSigningVerified);
            XCTAssertTrue(aliceDevice1Trust.isVerified);
...
{
   //  - Have Alice with 2 devices (Alice1 and Alice2) and Bob. All trusted via cross-signing
    [sendingnetworkSDKTestsE2EData doTestWithBobAndAliceWithTwoDevicesAllTrusted:self readyToTest:^(MXSession *aliceSession1, MXSession *aliceSession2, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        
        NSString *aliceUserId = aliceSession1.sendingnetworkRestClient.credentials.userId;
        NSString *aliceSession1DeviceId = aliceSession1.sendingnetworkRestClient.credentials.deviceId;
        NSString *aliceSession2DeviceId = aliceSession2.sendingnetworkRestClient.credentials.deviceId;
        
        NSString *bobUserId = bobSession.sendingnetworkRestClient.credentials.userId;
        NSString *bobDeviceId = bobSession.sendingnetworkRestClient.credentials.deviceId;
...
- (void)testTrustChain
{
    ...
    [sendingnetworkSDKTestsE2EData doTestWithBobAndAliceWithTwoDevicesAllTrusted:self readyToTest:^(MXSession *aliceSession1, MXSession *aliceSession2, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        // - Alice signs in on a new Device (Alice3)
        [sendingnetworkSDKTestsE2EData loginUserOnANewDevice:self credentials:aliceSession1.sendingnetworkRestClient.credentials withPassword:MXTESTS_ALICE_PWD onComplete:^(MXSession *aliceSession3) {
...
- (void)testMXCrossSigningMyUserDidSignInOnNewDeviceNotification
{
    ...
    [self doTestWithBobAndBootstrappedAlice:self readyToTest:^(MXSession *bobSession, MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [sendingnetworkSDKTestsE2EData loginUserOnANewDevice:self credentials:aliceSession.sendingnetworkRestClient.credentials withPassword:MXTESTS_ALICE_PWD onComplete:^(MXSession *newAliceSession) {
            newDeviceId = newAliceSession.sendingnetworkRestClient.credentials.deviceId;
        }];
