...
- (void)setUp
{
    ...
    self.sendingnetworkSDKTestsData = [[SendingnetworkSDKTestsData alloc] init];
    self.sendingnetworkSDKTestsE2EData = [[SendingnetworkSDKTestsE2EData alloc] initWithSendingnetworkSDKTestsData:self.sendingnetworkSDKTestsData];
}

- (void)tearDown
{
    self.sendingnetworkSDKTestsData = nil;
    self.sendingnetworkSDKTestsE2EData = nil;
}
...
- (void)testAnswerInAGappySync
{
    [self createScenarioForBobAndAlice:^(MXSession *bobSession, MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation, NSString *pollStartEventId, MXEventContentPollStart *pollStartContent) {
        ...
        // - Add enough messages while the session in background to trigger a gappy sync
        [bobSession pause];
        [self.sendingnetworkSDKTestsData for:bobSession.sendingnetworkRestClient andRoom:roomId sendMessages:10 testCase:self success:^{
            ...
            [aliceSession eventWithEventId:pollStartEventId inRoom:roomId success:^(MXEvent *event) {
                [aliceRoom sendPollResponseForEvent:event withAnswerIdentifiers:@[pollStartContent.answerOptions.lastObject.uuid] threadId:nil localEcho:nil success:^(NSString *eventId) {
                    ...
                    [self.sendingnetworkSDKTestsData for:bobSession.sendingnetworkRestClient andRoom:roomId sendMessages:20 testCase:self success:^{
                        [bobSession start:^{
                            [bobSession.aggregations referenceEventsForEvent:pollStartEventId inRoom:roomId from:nil limit:-1 success:^(MXAggregationPaginatedResponse *paginatedResponse) {
...
{
    [self.sendingnetworkSDKTestsData doMXSessionTestWithBobAndARoom:self andStore:[[MXMemoryStore alloc] init] readyToTest:^(MXSession *mxSession, MXRoom *room, XCTestExpectation *expectation) {
        ...
        [room sendPollStartWithContent:pollStartContent threadId:nil localEcho:nil success:^(NSString *pollStartEventId) {
            ...
            [mxSession.sendingnetworkRestClient eventWithEventId:pollStartEventId inRoom:room.roomId success:^(MXEvent *pollStartEvent) {
...
{
    [self.sendingnetworkSDKTestsData doTestWithAliceAndBobInARoom:self aliceStore:[[MXMemoryStore alloc] init] bobStore:[[MXMemoryStore alloc] init] readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [room sendPollStartWithContent:pollStartContent threadId:nil localEcho:nil success:^(NSString *pollStartEventId) {
            ...
            [bobSession.sendingnetworkRestClient eventWithEventId:pollStartEventId inRoom:room.roomId success:^(MXEvent *pollStartEvent) {
