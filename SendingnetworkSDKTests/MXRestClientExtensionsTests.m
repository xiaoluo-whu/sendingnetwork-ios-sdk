...
{
    SendingnetworkSDKTestsData *testData;
    SendingnetworkSDKTestsE2EData *e2eTestData;
}
...
- (void)setUp
{
    ...
    testData = [[SendingnetworkSDKTestsData alloc] init];
    e2eTestData = [[SendingnetworkSDKTestsE2EData alloc] initWithSendingnetworkSDKTestsData:testData];
}
...
- (void)testDownloadKeysByChunkForUsers
{
    ...
    [self createScenario:^(MXSession *aliceSession, MXSession *bobSession, MXSession *samSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        // - Get users keys in the normal way
        [aliceSession.sendingnetworkRestClient downloadKeysForUsers:userIds token:nil success:^(MXKeysQueryResponse *keysQueryResponse) {
            XCTAssertEqual(keysQueryResponse.deviceKeys.userIds.count, userIds.count);
            
            // - Get them from small chunks
            MXHTTPOperation *operation = [aliceSession.sendingnetworkRestClient downloadKeysByChunkForUsers:userIds token:nil chunkSize:1 success:^(MXKeysQueryResponse * _Nonnull chunkedKeysQueryResponse) {
...
- (void)testDownloadKeysByOneChunkForUsers
{
    ...
    [self createScenario:^(MXSession *aliceSession, MXSession *bobSession, MXSession *samSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [aliceSession.sendingnetworkRestClient downloadKeysForUsers:userIds token:nil success:^(MXKeysQueryResponse *keysQueryResponse) {
            ...
            // - Get them from a big chunk request
            MXHTTPOperation *operation = [aliceSession.sendingnetworkRestClient downloadKeysByChunkForUsers:userIds token:nil chunkSize:250 success:^(MXKeysQueryResponse * _Nonnull chunkedKeysQueryResponse) {
