...
{
    [sendingnetworkSDKTestsData doTestWithAliceAndBobInARoom:self aliceStore:[[MXMemoryStore alloc] init] bobStore:[[MXMemoryStore alloc] init] readyToTest:^(MXSession *mxSession, MXSession *otherSession, NSString *roomId, XCTestExpectation *expectation) {
...
- (void)createScenarioWithAGappySync:(void(^)(MXSession *mxSession, MXRoom *room, MXSession *otherSession, XCTestExpectation *expectation, NSString *eventId))readyToTest
{
    ...
    [self createScenario:^(MXSession *mxSession, MXRoom *room, MXSession *otherSession, XCTestExpectation *expectation, NSString *eventId) {
        ...
        // - Add enough messages while the session in background to trigger a gappy sync
        [mxSession pause];
        [sendingnetworkSDKTestsData for:mxSession.sendingnetworkRestClient andRoom:room.roomId sendMessages:10 testCase:self success:^{

            // - Add a reaction in the gap
            [otherSession.aggregations addReaction:@"🙂" forEvent:eventId inRoom:room.roomId success:^() {

                [sendingnetworkSDKTestsData for:mxSession.sendingnetworkRestClient andRoom:room.roomId sendMessages:20 testCase:self success:^{
...
- (void)createScenarioWithAGappyInitialSync:(void(^)(MXSession *mxSession, MXRoom *room, MXSession *otherSession, XCTestExpectation *expectation, NSString *eventId))readyToTest
{
    [self createScenarioWithAGappySync:^(MXSession *mxSession, MXRoom *room, MXSession *otherSession, XCTestExpectation *expectation, NSString *eventId) {
        ...
        // - Do an initial sync
        MXSession *mxSession2 = [[MXSession alloc] initWithSendingnetworkRestClient:restClient];
...
- (void)testAggregatedReactionServerSide
{
    ...
    [self createScenario:^(MXSession *mxSession, MXRoom *room, MXSession *otherSession, XCTestExpectation *expectation, NSString *eventId) {
        ...
        [mxSession.sendingnetworkRestClient eventWithEventId:eventId inRoom:room.roomId success:^(MXEvent *event) {
            ...
            // XCTAssertEqual(annotations.count, 1); // Not implemented on the backend - https://github.com/sending-network/synapse/issues/10557
