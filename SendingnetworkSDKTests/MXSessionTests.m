...
- (void)testInitialSyncSuccess
{
    ...
    [sendingnetworkSDKTestsData doMXSessionTestWithBob:self andStore:store readyToTest:^(MXSession *mxSession, XCTestExpectation *expectation) {
        MXCredentials *credentials = [MXCredentials initialSyncCacheCredentialsFrom:sendingnetworkSDKTestsData.bobCredentials];
...
- (void)testInitialSyncFailure
{
    ...
    [sendingnetworkSDKTestsData doMXRestClientTestWithBob:self readyToTest:^(MXRestClient *bobRestClient, XCTestExpectation *expectation) {

        MXCredentials *credentials = [MXCredentials initialSyncCacheCredentialsFrom:sendingnetworkSDKTestsData.bobCredentials];
        id<MXSyncResponseStore> cache = [[MXSyncResponseFileStore alloc] initWithCredentials:credentials];
        __block MXSession *mxSession = [[MXSession alloc] initWithSendingnetworkRestClient:bobRestClient];
        ...
        __block id observer = [[NSNotificationCenter defaultCenter] addObserverForName:kMXSessionStateDidChangeNotification object:nil queue:nil usingBlock:^(NSNotification * _Nonnull note) {
            ...
            if (mxSession.state == MXSessionStateRunning)
            {
                ...
                mxSession = [[MXSession alloc] initWithSendingnetworkRestClient:bobRestClient];
                [sendingnetworkSDKTestsData retain:mxSession];
...
- (void)testRoomWithAlias
{
    [sendingnetworkSDKTestsData doMXRestClientTestWithBob:self readyToTest:^(MXRestClient *bobRestClient, XCTestExpectation *expectation) {
        ...
        [bobRestClient createRoom:nil visibility:kMXRoomDirectoryVisibilityPrivate roomAlias:alias topic:nil success:^(MXCreateRoomResponse *response) {
            ...
            MXSession *mxSession = [[MXSession alloc] initWithSendingnetworkRestClient:bobRestClient];
            [sendingnetworkSDKTestsData retain:mxSession];
            [mxSession start:^{
...
{
    [sendingnetworkSDKTestsData doMXRestClientTestWihBobAndSeveralRoomsAndMessages:self readyToTest:^(MXRestClient *bobRestClient, XCTestExpectation *expectation) {
        ...
        [mxSession startWithSyncFilter:[MXFilterJSONModel syncFilterWithMessageLimit:0]
            ...
            [sendingnetworkSDKTestsData doMXRestClientTestWithBobAndARoomWithMessages:nil readyToTest:^(MXRestClient *bobRestClient, NSString *roomId, XCTestExpectation *expectation2) {
...
- (void)testListenerForRoomMessageOnly
{
    [sendingnetworkSDKTestsData doMXRestClientTestWihBobAndSeveralRoomsAndMessages:self readyToTest:^(MXRestClient *bobRestClient, XCTestExpectation *expectation) {
        ...
        [mxSession start:^{
            ...
            [sendingnetworkSDKTestsData doMXRestClientTestWithBobAndARoomWithMessages:nil readyToTest:^(MXRestClient *bobRestClient, NSString *roomId, XCTestExpectation *expectation) {
            }];
    ...
    [sendingnetworkSDKTestsData doMXRestClientTestWithBobAndAliceInARoom:self readyToTest:^(MXRestClient *bobRestClient, MXRestClient *aliceRestClient, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [mxSession listenToEventsOfTypes:@[kMXEventTypeStringPresence] onEvent:^(MXEvent *event, MXTimelineDirection direction, id customObject) {
            ...
            if (MXTimelineDirectionForwards == direction)
            {
                ...
                if (NO == [eventContent.userId isEqualToString:mxSession.sendingnetworkRestClient.credentials.userId])
                {
...
{
    [sendingnetworkSDKTestsData doMXRestClientTestWithBobAndARoomWithMessages:self readyToTest:^(MXRestClient *bobRestClient, NSString *roomId, XCTestExpectation *expectation) {
...
- (void)testState
{
    [sendingnetworkSDKTestsData doMXRestClientTestInABobRoomAndANewTextMessage:self newTextMessage:@"This is a text message for recents" onReadyToTest:^(MXRestClient *bobRestClient, NSString *roomId, NSString *new_text_message_eventId, XCTestExpectation *expectation) {
...
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndARoom:self andStore:[[MXMemoryStore alloc] init] readyToTest:^(MXSession *mxSession, MXRoom *room, XCTestExpectation *expectation) {
...
- (void)testCreateRoomWithInvite
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBob:self readyToTest:^(MXSession *mxSession, XCTestExpectation *expectation) {
        ...
        [sendingnetworkSDKTestsData doMXRestClientTestWithAlice:nil readyToTest:^(MXRestClient *aliceRestClient, XCTestExpectation *expectation2) {
            ...
            parameters.inviteArray = @[sendingnetworkSDKTestsData.aliceCredentials.userId];
            ...
            [mxSession createRoomWithParameters:parameters success:^(MXRoom *room) {
                ...
                [mxSession.sendingnetworkRestClient membersOfRoom:room.roomId success:^(NSArray *roomMemberEvents) {
                    ...
                    {
                        succeed = [roomMemberEvent2.stateKey isEqualToString:sendingnetworkSDKTestsData.aliceCredentials.userId];
                    }
                    else if ([roomMemberEvent1.stateKey isEqualToString:sendingnetworkSDKTestsData.aliceCredentials.userId])
                    {
...
- (void)testCreateDirectRoom
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBob:self readyToTest:^(MXSession *mxSession, XCTestExpectation *expectation) {
        ...
        [sendingnetworkSDKTestsData doMXRestClientTestWithAlice:nil readyToTest:^(MXRestClient *aliceRestClient, XCTestExpectation *expectation2) {
            ...
            // Create a random room with no params
            MXRoomCreationParameters *parameters = [MXRoomCreationParameters parametersForDirectRoomWithUser:sendingnetworkSDKTestsData.aliceCredentials.userId];
            [mxSession createRoomWithParameters:parameters success:^(MXRoom *room) {
                ...
                [mxSession.sendingnetworkRestClient membersOfRoom:room.roomId success:^(NSArray *roomMemberEvents) {
                    ...
                    [mxSession startWithSyncFilter:[MXFilterJSONModel syncFilterWithMessageLimit:0]
                        ...
                        [room state:^(MXRoomState *roomState) {
                            ...
                            NSUInteger powerlLevel2 = [roomPowerLevels powerLevelOfUserWithUserID:sendingnetworkSDKTestsData.aliceCredentials.userId];
...
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndAliceInARoom:self readyToTest:^(MXSession *mxSession, MXRestClient *aliceRestClient, NSString *roomId, XCTestExpectation *expectation) {
...
{
    [sendingnetworkSDKTestsData doMXRestClientTestWithBobAndARoom:self readyToTest:^(MXRestClient *bobRestClient, NSString *roomId, XCTestExpectation *expectation) {

        [sendingnetworkSDKTestsData doMXRestClientTestWithAlice:nil readyToTest:^(MXRestClient *aliceRestClient, XCTestExpectation *expectation2) {

            MXSession *mxSession = [[MXSession alloc] initWithSendingnetworkRestClient:aliceRestClient];
            [sendingnetworkSDKTestsData retain:mxSession];
            [mxSession start:^{
...
- (void)testNewRoomNotificationOnCreatingPublicRoom
{
    [sendingnetworkSDKTestsData doMXRestClientTestWithBob:self readyToTest:^(MXRestClient *bobRestClient, XCTestExpectation *expectation) {
        ...
        [mxSession start:^{
            ...
            [mxSession.sendingnetworkRestClient createRoom:nil visibility:kMXRoomDirectoryVisibilityPublic roomAlias:nil topic:nil success:^(MXCreateRoomResponse *response) {
...
{
    [sendingnetworkSDKTestsData doMXRestClientTestWithBobAndAPublicRoom:self readyToTest:^(MXRestClient *bobRestClient, NSString *roomId, XCTestExpectation *expectation) {
...
- (void)testInvitedRooms
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndAliceInARoom:self readyToTest:^(MXSession *mxSession, MXRestClient *aliceRestClient, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [aliceRestClient createRoom:@"A room name" visibility:kMXRoomDirectoryVisibilityPrivate roomAlias:nil topic:nil success:^(MXCreateRoomResponse *response) {
            ...
            [aliceRestClient inviteUser:mxSession.sendingnetworkRestClient.credentials.userId toRoom:testRoomId success:^{
...
- (void)testMXSDKOptionsWellknownDomainUrl
{
    ...
    [HTTPStubs stubRequestsPassingTest:^BOOL(NSURLRequest *request) {
        if ([request.URL.absoluteString containsString:@".well-known/sendingnetwork/client"])
        {
            // -> The wellknown request must be done on the custom domain
    ...
    // - Set up a MXSession
    [sendingnetworkSDKTestsData doMXSessionTestWithAlice:self readyToTest:^(MXSession *aliceSession, XCTestExpectation *theExpectation) {
