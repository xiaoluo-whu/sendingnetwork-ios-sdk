...
#import "SendingnetworkSDKTestsE2EData.h"
...
@interface MXCryptoTests : XCTestCase
{
    ...
    SendingnetworkSDKTestsE2EData *sendingnetworkSDKTestsE2EData;
...
- (void)setUp
{
    ...
    sendingnetworkSDKTestsE2EData = [[SendingnetworkSDKTestsE2EData alloc] initWithSendingnetworkSDKTestsData:sendingnetworkSDKTestsData];
}
...
- (void)tearDown
{
    ...
    sendingnetworkSDKTestsE2EData = nil;
...
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBob:self readyToTest:^(MXSession *mxSession, XCTestExpectation *expectation) {

        XCTAssertNil(mxSession.crypto, @"Crypto is disabled by default");

        XCTAssertFalse([mxSession.crypto.store.class hasDataForCredentials:mxSession.sendingnetworkRestClient.credentials]);

        [mxSession enableCrypto:YES success:^{

            XCTAssert(mxSession.crypto);
            XCTAssert([mxSession.crypto.store.class hasDataForCredentials:mxSession.sendingnetworkRestClient.credentials]);

            [mxSession enableCrypto:NO success:^{

                XCTAssertNil(mxSession.crypto);
                XCTAssertFalse([mxSession.crypto.store.class hasDataForCredentials:mxSession.sendingnetworkRestClient.credentials], @"Crypto data must have been trashed");
...
- (void)testCryptoNoDeviceId
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBob:self readyToTest:^(MXSession *mxSession, XCTestExpectation *expectation) {
        ...
        mxSession.sendingnetworkRestClient.credentials.deviceId = nil;
...
- (void)testCryptoPersistenceInStore
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBob:self readyToTest:^(MXSession *mxSession, XCTestExpectation *expectation) {
        ...
        [mxSession enableCrypto:YES success:^{
            ...
            MXRestClient *bobRestClient = mxSession2.sendingnetworkRestClient;
            [mxSession2 close];
            mxSession2 = nil;
            ...
            mxSession2 = [[MXSession alloc] initWithSendingnetworkRestClient:bobRestClient];
            [sendingnetworkSDKTestsData retain:mxSession2];
...
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithBobAndAlice:self readyToTest:^(MXSession *bobSession, MXSession *aliceSession, XCTestExpectation *expectation) {
...
- (void)testDownloadKeysForUserWithNoDevice
{
    ...
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoom:self cryptedBob:NO warnOnUnknowDevices:NO readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
...
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithBobAndAlice:self readyToTest:^(MXSession *bobSession, MXSession *aliceSession, XCTestExpectation *expectation) {
        // Try to get info from a user on sending.network.
        // The local hs we use for tests is not federated and is not able to talk with sending.network
        [aliceSession.crypto.deviceList downloadKeys:@[bobSession.myUser.userId, @"@auser:sending.network"] forceDownload:NO success:^(MXUsersDevicesMap<MXDeviceInfo *> *usersDevicesInfoMap, NSDictionary<NSString *,MXCrossSigningInfo *> *crossSigningKeysMap) {
...
- (void)testMXSessionEventWithEventId
{
    ...
    // - Have Alice with an encrypted message
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceInARoom:self readyToTest:^(MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation) {
...
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceInARoom:self
...
- (void)testAliceInACryptedRoomAfterInitialSync
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceInARoom:self readyToTest:^(MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        MXRestClient *aliceRestClient = aliceSession.sendingnetworkRestClient;
        [aliceSession close];
        aliceSession = nil;

        MXSession *aliceSession2 = [[MXSession alloc] initWithSendingnetworkRestClient:aliceRestClient];
        [sendingnetworkSDKTestsData retain:aliceSession2];
...
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoom:self cryptedBob:YES warnOnUnknowDevices:NO readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
    ...
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoom:self cryptedBob:YES warnOnUnknowDevices:NO readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [roomFromBobPOV liveTimeline:^(id<MXEventTimeline> liveTimeline) {
            [liveTimeline listenToEventsOfTypes:@[kMXEventTypeStringRoomMessage] onEvent:^(MXEvent *event, MXTimelineDirection direction, MXRoomState *roomState) {
                ...
                XCTAssertEqual(0, [self checkEncryptedEvent:event roomId:roomId clearMessage:sendingnetworkSDKTestsE2EData.messagesFromAlice[receivedMessagesFromAlice++] senderSession:aliceSession]);
                ...
                switch (receivedMessagesFromAlice)
                {
                    case 1:
                    {
                        ...
                        [roomFromBobPOV sendTextMessage:sendingnetworkSDKTestsE2EData.messagesFromBob[0] threadId:nil success:^(NSString *eventId) {
                            [roomFromBobPOV sendTextMessage:sendingnetworkSDKTestsE2EData.messagesFromBob[1] threadId:nil success:^(NSString *eventId) {
                                [roomFromBobPOV sendTextMessage:sendingnetworkSDKTestsE2EData.messagesFromBob[2] threadId:nil success:nil failure:nil];
                            } failure:nil];
                        } failure:nil];
        ...
        [roomFromAlicePOV liveTimeline:^(id<MXEventTimeline> liveTimeline) {
            [liveTimeline listenToEventsOfTypes:@[kMXEventTypeStringRoomMessage] onEvent:^(MXEvent *event, MXTimelineDirection direction, MXRoomState *roomState) {
                ...
                XCTAssertEqual(0, [self checkEncryptedEvent:event roomId:roomId clearMessage:sendingnetworkSDKTestsE2EData.messagesFromBob[receivedMessagesFromBob++] senderSession:bobSession]);

                if (receivedMessagesFromBob == 3)
                {
                    [roomFromAlicePOV sendTextMessage:sendingnetworkSDKTestsE2EData.messagesFromAlice[1] threadId:nil success:nil failure:nil];
                }
            }];
        }];

        [roomFromAlicePOV sendTextMessage:sendingnetworkSDKTestsE2EData.messagesFromAlice[0] threadId:nil success:nil failure:^(NSError *error) {
            XCTFail(@"Cannot set up intial test conditions - error: %@", error);
            [expectation fulfill];
        }];
    }];
...
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoomWithCryptedMessages:self cryptedBob:YES readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {

        MXRestClient *bobRestClient = bobSession.sendingnetworkRestClient;

        [bobSession close];
        bobSession = nil;

        bobSession = [[MXSession alloc] initWithSendingnetworkRestClient:bobRestClient];
        [sendingnetworkSDKTestsData retain:bobSession];
            ...
            [bobSession start:^{
                ...
                [roomFromBobPOV liveTimeline:^(id<MXEventTimeline> liveTimeline) {
                    ...
                    [liveTimeline listenToEventsOfTypes:@[kMXEventTypeStringRoomMessage] onEvent:^(MXEvent *event, MXTimelineDirection direction, MXRoomState *roomState) {
                        ...
                        {
                            case 0:
                                XCTAssertEqual(0, [self checkEncryptedEvent:event roomId:roomId clearMessage:sendingnetworkSDKTestsE2EData.messagesFromAlice[1] senderSession:aliceSession]);
                                break;

                            case 1:
                                XCTAssertEqual(0, [self checkEncryptedEvent:event roomId:roomId clearMessage:sendingnetworkSDKTestsE2EData.messagesFromBob[2] senderSession:bobSession]);
                                break;

                            case 2:
                                XCTAssertEqual(0, [self checkEncryptedEvent:event roomId:roomId clearMessage:sendingnetworkSDKTestsE2EData.messagesFromBob[1] senderSession:bobSession]);
                                break;

                            case 3:
                                XCTAssertEqual(0, [self checkEncryptedEvent:event roomId:roomId clearMessage:sendingnetworkSDKTestsE2EData.messagesFromBob[0] senderSession:bobSession]);
                                break;

                            case 4:
                                XCTAssertEqual(0, [self checkEncryptedEvent:event roomId:roomId clearMessage:sendingnetworkSDKTestsE2EData.messagesFromAlice[0] senderSession:aliceSession]);
                                break;
...
- (void)testAliceDecryptOldMessageWithANewDeviceInACryptedRoom
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceInARoom:self readyToTest:^(MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [roomFromAlicePOV sendTextMessage:message threadId:nil success:^(NSString *eventId) {
            ...
            [MXSDKOptions sharedInstance].enableCryptoWhenStartingMXSession = YES;
            [sendingnetworkSDKTestsData relogUserSession:self session:aliceSession withPassword:MXTESTS_ALICE_PWD onComplete:^(MXSession *aliceSession2) {
                [MXSDKOptions sharedInstance].enableCryptoWhenStartingMXSession = NO;
    ...
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoom:self cryptedBob:YES warnOnUnknowDevices:NO readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [roomFromBobPOV liveTimeline:^(id<MXEventTimeline> liveTimeline) {
            [liveTimeline listenToEventsOfTypes:@[kMXEventTypeStringRoomMessage, kMXEventTypeStringRoomEncrypted] onEvent:^(MXEvent *event, MXTimelineDirection direction, MXRoomState *roomState) {
                ...
                [MXSDKOptions sharedInstance].enableCryptoWhenStartingMXSession = YES;
                [sendingnetworkSDKTestsData relogUserSession:self session:bobSession withPassword:MXTESTS_BOB_PWD onComplete:^(MXSession *bobSession2) {
                    [MXSDKOptions sharedInstance].enableCryptoWhenStartingMXSession = NO;
                ...
                switch (messageCount++)
                {
                    case 0:
                    {
                        ...
                        [aliceSession.crypto setDeviceVerification:MXDeviceBlocked
                                                         forDevice:bobSession.sendingnetworkRestClient.credentials.deviceId
...
- (void)testBlackListUnverifiedDevices
{
    ...
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobAndSamInARoom:self cryptedBob:YES cryptedSam:YES warnOnUnknowDevices:YES readyToTest:^(MXSession *aliceSession, MXSession *bobSession, MXSession *samSession, NSString *roomId, XCTestExpectation *expectation) {
...
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceByInvitingBobInARoom:self cryptedBob:YES warnOnUnknowDevices:NO readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [aliceSession.sendingnetworkRestClient setRoomHistoryVisibility:roomId historyVisibility:kMXRoomHistoryVisibilityInvited success:^{
...
- (void)testInvitedMemberInACryptedRoom2
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceByInvitingBobInARoom:self cryptedBob:YES warnOnUnknowDevices:NO readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [aliceSession.sendingnetworkRestClient setRoomHistoryVisibility:roomId historyVisibility:kMXRoomHistoryVisibilityJoined success:^{
...
// Inspired from https://github.com/poljar/sendingnetwork-nio/blob/0.7.1/tests/encryption_test.py#L872
//
...
- (void)testOlmSessionUnwedging
{
    ...
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoom:self cryptedBob:YES warnOnUnknowDevices:NO aliceStore:[[MXFileStore alloc] init] bobStore:[[MXFileStore alloc] init] readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [roomFromAlicePOV sendTextMessage:@"0" threadId:nil success:^(NSString *eventId) {
            ...
            MXSession *aliceSession1 = [[MXSession alloc] initWithSendingnetworkRestClient:aliceSession.sendingnetworkRestClient];
            ...
            [aliceSession1 setStore:[[MXFileStore alloc] init] success:^{
                [aliceSession1 start:^{
                    ...
                    [roomFromAlicePOV1 sendTextMessage:@"11" threadId:nil success:^(NSString *eventId) {
                        ...
                        MXSession *aliceSession2 = [[MXSession alloc] initWithSendingnetworkRestClient:aliceSession1.sendingnetworkRestClient];
                        [sendingnetworkSDKTestsData retain:aliceSession2];
...
// Test for https://github.com/sending-network/sendingnetwork-js-sdk/pull/359
            ...
            [bobSession joinRoom:roomFromAlicePOV.roomId viaServers:nil success:^(MXRoom *room) {
                ...
                [roomFromAlicePOV sendTextMessage:messageFromAlice threadId:nil success:^(NSString *eventId) {
                    ...
                    [bobSession enableCrypto:NO success:^{
                        ...
                        // Relog bob to simulate a new device
                        [MXSDKOptions sharedInstance].enableCryptoWhenStartingMXSession = YES;
                        [sendingnetworkSDKTestsData relogUserSession:self session:bobSession withPassword:MXTESTS_BOB_PWD onComplete:^(MXSession *newBobSession) {
...
//                    [aliceSession close];
//
//                    aliceSession2 = [[MXSession alloc] initWithSendingnetworkRestClient:aliceRestClient];
//                    [sendingnetworkSDKTestsData retain:aliceSession2];
//
//                    [aliceSession2 start:^{
//
...
//                    [aliceSession2 close];
//
//                    aliceSession3 = [[MXSession alloc] initWithSendingnetworkRestClient:aliceRestClient];
//                    [sendingnetworkSDKTestsData retain:aliceSession3];
//
//                    [aliceSession3 start:^{
//
    ...
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoomWithCryptedMessages:self cryptedBob:YES readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        [bobSession.crypto exportRoomKeys:^(NSArray<NSDictionary *> *keys) {
            ...
            [bobSession enableCrypto:NO success:^{
                ...
                XCTAssertFalse([bobSession.crypto.store.class hasDataForCredentials:bobSession.sendingnetworkRestClient.credentials], @"Bob's keys should have been deleted");
...
/*
 ...
 5 - Instantiante a MXRestclient, alice1SendingnetworkRestClient, with the credentials of
     the 1st device (kept at step #2)
 6 - Make alice1SendingnetworkRestClient make a fake room key request for the message sent at step #4
...
- (void)testIncomingRoomKeyRequest
{
    ...
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceInARoom:self readyToTest:^(MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        // 2 - Close it by keeping her credentials
        MXCredentials *alice1Credentials = aliceSession.sendingnetworkRestClient.credentials;

        // 3 - Recreate a second MXSession, aliceSession2, for Alice with a new device
        // Relog alice to simulate a new device
        [MXSDKOptions sharedInstance].enableCryptoWhenStartingMXSession = YES;
        [sendingnetworkSDKTestsData relogUserSessionWithNewDevice:self session:aliceSession withPassword:MXTESTS_ALICE_PWD onComplete:^(MXSession *aliceSession2) {
            [MXSDKOptions sharedInstance].enableCryptoWhenStartingMXSession = NO;
            ...
            [roomFromAlice2POV liveTimeline:^(id<MXEventTimeline> liveTimeline) {
                [liveTimeline listenToEventsOfTypes:@[kMXEventTypeStringRoomMessage, kMXEventTypeStringRoomEncrypted] onEvent:^(MXEvent *event, MXTimelineDirection direction, MXRoomState *roomState) {
                    ...
                    // 5 - Instantiante a MXRestclient, alice1SendingnetworkRestClient
                    MXRestClient *alice1SendingnetworkRestClient = [[MXRestClient alloc] initWithCredentials:alice1Credentials andOnUnrecognizedCertificateBlock:nil andPersistentTokenDataHandler:nil andUnauthenticatedHandler:nil];
                    [sendingnetworkSDKTestsData retain:alice1SendingnetworkRestClient];
                    ...
                    [alice1SendingnetworkRestClient sendToDevice:kMXEventTypeStringRoomKeyRequest contentMap:contentMap txnId:requestMessage[@"request_id"] success:nil failure:^(NSError *error) {
                        XCTFail(@"The operation should not fail - NSError: %@", error);
                        [expectation fulfill];
                    }];
...
/**
 ...
 -        dispatch_async(self->crypto.sendingnetworkRestClient.completionQueue, ^{
 +        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 1 * NSEC_PER_SEC), self->crypto.sendingnetworkRestClient.completionQueue, ^{
...
- (void)testMXDeviceListDidUpdateUsersDevicesNotification
{
    ...
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceInARoom:self readyToTest:^(MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        // - Alice logs in on a new device
        [sendingnetworkSDKTestsE2EData loginUserOnANewDevice:self credentials:aliceSession.sendingnetworkRestClient.credentials withPassword:MXTESTS_ALICE_PWD onComplete:^(MXSession *newAliceSession) {
        }];
...
- (void)testRestoreOlmOutboundKey
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceAndBobInARoomWithCryptedMessages:self cryptedBob:YES readyToTest:^(MXSession *aliceSession, MXSession *bobSession, NSString *roomId, XCTestExpectation *expectation) {
        ...
        // - Restart the session
        MXSession *aliceSession2 = [[MXSession alloc] initWithSendingnetworkRestClient:aliceSession.sendingnetworkRestClient];
        [sendingnetworkSDKTestsData retain:aliceSession2];
...
- (void)testFallbackKeySignatures
{
    ...
    [sendingnetworkSDKTestsData doMXSessionTestWithBob:self readyToTest:^(MXSession *mxSession, XCTestExpectation *expectation) {
        ...
        [usersDevicesKeyTypesMap setObject:@"curve25519"
                                   forUser:mxSession.sendingnetworkRestClient.credentials.userId
                                 andDevice:mxSession.sendingnetworkRestClient.credentials.deviceId];
...
// For dev purpose, it is interesting to comment https://github.com/sending-network/sendingnetwork-ios-sdk/blob/610db96cf8e470770f92d6afc40bc4332b240da4/SendingnetworkSDK/Data/MXRoomSummary.m#L552
//
...
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceInARoom:self readyToTest:^(MXSession *session, NSString *roomId, XCTestExpectation *expectation) {
