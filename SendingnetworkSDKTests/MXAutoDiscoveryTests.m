...
static NSString *const kWellKnowPath = @".well-known/api/client";
static NSString *const kVersionPath = @"_api/client/versions";
static NSString *const kIdentityServerPingPath = @"_api/identity/api/v1";
...
// Same test as testAutoDiscoverySuccessful but with .well-known/sendingnetwork/client
// not returned with a JSON content type.
// This is what happens on sending.network
...
// Test on sending.network
// Only for development. Must be disabled for automatic tests
//- (void)testAutoDiscoveryWithSendingnetworkDotOrg
//{
//    XCTestExpectation *expectation = [self expectationWithDescription:@"asyncTest"];
//
//    MXAutoDiscovery *autoDiscovery = [[MXAutoDiscovery alloc] initWithDomain:@"sending.network"];
//    [autoDiscovery findClientConfig:^(MXDiscoveredClientConfig * _Nonnull discoveredClientConfig) {
//
//        XCTAssertEqual(discoveredClientConfig.action, MXDiscoveredClientConfigActionPrompt);
//        XCTAssertEqualObjects(discoveredClientConfig.wellKnown.homeServer.baseUrl, @"https://sending.network");
