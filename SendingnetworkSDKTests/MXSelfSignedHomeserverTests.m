...
- (void)testRegiter
{
    ...
    [sendingnetworkSDKTestsData getHttpsBobCredentials:self readyToTest:^{

        XCTAssert(certificateCheckAsked, @"We must have been asked to check the certificate");
        XCTAssertNotNil(sendingnetworkSDKTestsData.bobCredentials);

        XCTAssertNotNil(sendingnetworkSDKTestsData.bobCredentials.accessToken);
        XCTAssertNotNil(sendingnetworkSDKTestsData.bobCredentials.allowedCertificate);
        XCTAssertNil(sendingnetworkSDKTestsData.bobCredentials.ignoredCertificate);
...
- (void)testTrustedCertificate
{
    ...
    [sendingnetworkSDKTestsData getHttpsBobCredentials:self readyToTest:^{
        ...
        MXRestClient *mxRestClient = [[MXRestClient alloc] initWithCredentials:sendingnetworkSDKTestsData.bobCredentials andOnUnrecognizedCertificateBlock:^BOOL(NSData *certificate) {
...
{
    [sendingnetworkSDKTestsData doHttpsMXSessionTestWithBob:self readyToTest:^(MXSession *mxSession, XCTestExpectation *expectation) {
        ...
        [mxSession createRoom:@"A room" visibility:0 roomAlias:nil topic:nil success:^(MXRoom *room) {
            ...
            [room liveTimeline:^(id<MXEventTimeline> liveTimeline) {
                ...
                [liveTimeline listenToEventsOfTypes:@[kMXEventTypeStringRoomMessage] onEvent:^(MXEvent *event, MXTimelineDirection direction, MXRoomState *roomState) {
                    ...
                    [mxSession.mediaManager downloadMediaFromSendingnetworkContentURI:contentURL
                                                                     withType:nil
                                                                     inFolder:nil
        ...
        [mxSession createRoom:@"A room" visibility:0 roomAlias:nil topic:nil success:^(MXRoom *room) {
            ...
            [room liveTimeline:^(id<MXEventTimeline> liveTimeline) {
                ...
                [liveTimeline listenToEventsOfTypes:@[kMXEventTypeStringRoomMessage] onEvent:^(MXEvent *event, MXTimelineDirection direction, MXRoomState *roomState) {
                    ...
                    MXMediaManager *mediaManager = [[MXMediaManager alloc] initWithHomeServer:mxSession.sendingnetworkRestClient.homeserver];
                    ...
                    [mediaManager downloadMediaFromSendingnetworkContentURI:contentURL
                                                                     withType:nil
                                                                     inFolder:nil
                                                                      success:^(NSString *outputFilePath) {
