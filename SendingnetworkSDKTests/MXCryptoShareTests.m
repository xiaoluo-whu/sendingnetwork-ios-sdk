...
- (void)mimicKeyShareResponseForSession:(MXSession*)session withSessionData:(MXMegolmSessionData*)sessionData complete:(void (^)(void))complete
{
    [session.crypto importMegolmSessionDatas:@[sessionData] backUp:NO success:^(NSUInteger total, NSUInteger imported) {
        ...
        [sendingnetworkSDKTestsData breakTestCase:self reason:@"Cannot set up intial test conditions - error: %@", error];
    }];
}
            ...
            [liveTimeline paginate:30 direction:MXTimelineDirectionBackwards onlyFromStore:NO complete:^{
                ...
                [sendingnetworkSDKTestsE2EData outgoingRoomKeyRequestInSession:aliceSession complete:^(MXOutgoingRoomKeyRequest *outgoingRoomKeyRequest) {
                    ...
                    [self mimicKeyShareResponseForSession:aliceSession withSessionData:partialSessionData complete:^{
                        ...
                        // -> The outgoing key share request should still exist
                        [sendingnetworkSDKTestsE2EData outgoingRoomKeyRequestInSession:aliceSession complete:^(MXOutgoingRoomKeyRequest *pendingOutgoingRoomKeyRequest) {
                            ...
                            [self mimicKeyShareResponseForSession:aliceSession withSessionData:sessionData complete:^{
                                ...
                                [sendingnetworkSDKTestsE2EData outgoingRoomKeyRequestInSession:aliceSession complete:^(MXOutgoingRoomKeyRequest *stillPendingOutgoingRoomKeyRequest) {
...
- (void)testShareHistoryKeysWithInvitedUser
{
    [sendingnetworkSDKTestsE2EData doE2ETestWithAliceInARoom:self
                                         ...
                                         readyToTest:^(MXSession *aliceSession, NSString *roomId, XCTestExpectation *expectation)
     {
        ...
        [MXSDKOptions sharedInstance].enableCryptoWhenStartingMXSession = YES;
        [sendingnetworkSDKTestsData doMXSessionTestWithBob:nil
                    ...
                    [self sendMessage:@"Hi" room:room success:^{
                        [self setHistoryVisibility:kMXRoomHistoryVisibilityShared room:room success:^{
                            [self sendMessage:@"How are you?" room:room success:^{
                                ...
                                // Alice signs in on a new device
                                [sendingnetworkSDKTestsE2EData loginUserOnANewDevice:self
                                                                 credentials:aliceSession1.sendingnetworkRestClient.credentials
