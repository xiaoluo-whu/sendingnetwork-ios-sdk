...
- (void)testListenerForAllLiveEvents
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndThePublicRoom:self readyToTest:^(MXSession *mxSession, MXRoom *room, XCTestExpectation *expectation) {
        ...
        [sendingnetworkSDKTestsData doMXRestClientTestWithBobAndThePublicRoom:nil readyToTest:^(MXRestClient *bobRestClient, NSString *roomId, XCTestExpectation *expectation2) {
...
{
    [sendingnetworkSDKTestsData doMXRestClientTestWithBobAndARoom:self readyToTest:^(MXRestClient *bobClient, NSString *roomId, XCTestExpectation *expectation) {

        [sendingnetworkSDKTestsData doMXRestClientTestWithAlice:nil readyToTest:^(MXRestClient *aliceClient, XCTestExpectation *expectation2) {

            MXSession *mxSession = [[MXSession alloc] initWithSendingnetworkRestClient:aliceClient];
...
- (void)testTagEvent
{
    [sendingnetworkSDKTestsData doMXRestClientTestInABobRoomAndANewTextMessage:self newTextMessage:@"This is a text message for tagged events" onReadyToTest:^(MXRestClient *bobRestClient, NSString *roomId, NSString *new_text_message_eventId, XCTestExpectation *expectation) {
...
- (void)testUpgrade
{
    [sendingnetworkSDKTestsData doMXSessionTestWithBobAndARoomWithMessages:self readyToTest:^(MXSession *mxSession, MXRoom *room, XCTestExpectation *expectation) {
        ...
        [mxSession.sendingnetworkRestClient upgradeRoomWithId:room.roomId to:newRoomVersion success:^(NSString *replacementRoomId) {
