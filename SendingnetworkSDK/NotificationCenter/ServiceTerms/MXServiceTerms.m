 ...
 */
NSString *const MXServiceTermsErrorDomain = @"org.sendingnetwork.sdk.MXServiceTermsErrorDomain";
...
- (instancetype)initWithBaseUrl:(NSString*)baseUrl serviceType:(MXServiceType)serviceType sendingnetworkSession:(nullable MXSession *)mxSession accessToken:(nullable NSString *)accessToken
{
    self = [super init];
    if (self)
    {
                          ...
                          failure:(void (^)(NSError * _Nonnull))failure
{
    if (!_mxSession || !_accessToken)
    {
        if (failure)
        {
            NSError *error = [NSError errorWithDomain:MXServiceTermsErrorDomain code:MXServiceTermsErrorMissingParameters userInfo:@{ NSLocalizedDescriptionKey : @"No Sendingnetwork session or no access token"}];
            failure(error);
        }
        return nil;
    }
