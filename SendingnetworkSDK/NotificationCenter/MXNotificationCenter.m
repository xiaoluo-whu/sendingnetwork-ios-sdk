...
{
    /**
     The Sendingnetwork session to make be able to make Home Server requests.
     */
    MXSession *mxSession;
...
- (instancetype)initWithSendingnetworkSession:(MXSession *)mxSession2
{
    self = [super init];
    if (self)
    {
        ...
        // Define condition checkers for default Sendingnetwork conditions
        ...
        MXPushRuleDisplayNameCondtionChecker *displayNameCondtionChecker = [[MXPushRuleDisplayNameCondtionChecker alloc] initWithSendingnetworkSession:mxSession currentUserDisplayName:nil];
        [self setChecker:displayNameCondtionChecker forConditionKind:kMXPushRuleConditionStringContainsDisplayName];

        MXPushRuleRoomMemberCountConditionChecker *roomMemberCountConditionChecker = [[MXPushRuleRoomMemberCountConditionChecker alloc] initWithSendingnetworkSession:mxSession];
        [self setChecker:roomMemberCountConditionChecker forConditionKind:kMXPushRuleConditionStringRoomMemberCount];

        MXPushRuleSenderNotificationPermissionConditionChecker *notificationPermissionConditionChecker = [[MXPushRuleSenderNotificationPermissionConditionChecker alloc] initWithSendingnetworkSession:mxSession];
        ...
        [mxSession listenToEvents:^(MXEvent *event, MXTimelineDirection direction, id customObject) {
            ...
            if (MXTimelineDirectionForwards == direction
                && NO == [event.sender isEqualToString:self->mxSession.sendingnetworkRestClient.credentials.userId])
            {
...
{
    return [mxSession.sendingnetworkRestClient pushRules:^(MXPushRulesResponse *pushRules) {
...
- (MXPushRule *)ruleMatchingEvent:(MXEvent *)event roomState:(MXRoomState*)roomState
{
    ...
    if (NO == [event.sender isEqualToString:mxSession.sendingnetworkRestClient.credentials.userId])
    {
...
- (void)removeRule:(MXPushRule*)pushRule
{
    if (pushRule)
    {
        ...
        MXWeakify(self);
        [mxSession.sendingnetworkRestClient removePushRule:pushRule.ruleId scope:pushRule.scope kind:pushRule.kind success:^{
            MXStrongifyAndReturnIfNil(self);
...
- (void)enableRule:(MXPushRule*)pushRule isEnabled:(BOOL)enable
{
    if (pushRule)
    {
        ...
        MXWeakify(self);
        [mxSession.sendingnetworkRestClient enablePushRule:pushRule.ruleId scope:pushRule.scope kind:pushRule.kind enable:enable success:^{
            MXStrongifyAndReturnIfNil(self);
                    ...
                    highlight:(BOOL)highlight
{
    ...
    [mxSession.sendingnetworkRestClient updateActionsForPushRule:ruleId
            ...
            highlight:(BOOL)highlight
{
    ...
    [mxSession.sendingnetworkRestClient addPushRule:ruleId scope:kMXPushRuleScopeStringGlobal kind:kind actions:actions pattern:pattern conditions:conditions success:^{
