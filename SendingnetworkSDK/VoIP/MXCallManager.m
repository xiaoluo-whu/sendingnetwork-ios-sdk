...
@interface MXCallManager ()
{
    ...
    /**
     Listener to Sendingnetwork call-related events.
     */
...
- (instancetype)initWithSendingnetworkSession:(MXSession *)mxSession andCallStack:(id<MXCallStack>)callstack
{
    self = [super init];
    if (self)
    {
...
{
    MXWeakify(self);
    [_mxSession.sendingnetworkRestClient turnServer:^(MXTurnServerResponse *turnServerResponse) {
        MXStrongifyAndReturnIfNil(self);
...
// FIXME: This currently forces Vector to try to hit the sending.network AS for conferencing.
// This is bad because it prevents people running their own ASes from being used.
// This isn't permanent and will be customisable in the future: see the proposal
// at docs/conferencing.md for more info.
NSString *const kMXCallManagerConferenceUserPrefix  = @"@fs_";
NSString *const kMXCallManagerConferenceUserDomain  = @"sending.network";
...
{
    // Apply the same algo as other sendingnetwork clients
    ...
    if ([userId hasPrefix:kMXCallManagerConferenceUserPrefix])
    {
        ...
        if (base64part)
        {
            ...
            if (decodedData)
            {
                ...
                if (decoded)
                {
                    isConferenceUser = [MXTools isSendingnetworkRoomIdentifier:decoded];
                }
            }
        }
    }
...
{
    MXWeakify(self);
    [_mxSession.sendingnetworkRestClient thirdpartyProtocols:^(MXThirdpartyProtocolsResponse *response) {
        MXStrongifyAndReturnIfNil(self);
...
{
    [_mxSession.sendingnetworkRestClient thirdpartyUsers:kMXProtocolVectorPSTN
                                          fields:@{
...
- (void)getVirtualUserFrom:(NSString *)userId
                   ...
                   failure:(void (^)(NSError * _Nullable))failure
{
    [_mxSession.sendingnetworkRestClient thirdpartyUsers:kMXProtocolVectorSipVirtual
                                          fields:@{
                                              @"native_mxid": userId
                                          }
                                         success:^(MXThirdPartyUsersResponse *thirdpartyUsersResponse) {
...
- (void)getNativeUserFrom:(NSString *)userId
                  ...
                  failure:(void (^)(NSError * _Nullable))failure
{
    [_mxSession.sendingnetworkRestClient thirdpartyUsers:kMXProtocolVectorSipNative
                                          fields:@{
                                              @"virtual_mxid": userId
                                          }
                                         success:^(MXThirdPartyUsersResponse *thirdpartyUsersResponse) {
