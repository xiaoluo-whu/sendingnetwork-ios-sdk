...
#if __has_include(<SendingnetworkKit/SendingnetworkKit.h>)
#import <SendingnetworkKit/SendingnetworkKit.h>
#endif
...
-(void)sendBugReport:(NSString *)text sendFiles:(NSArray<NSURL*>*)files attachGitHubLabels:(NSArray<NSString*>*)gitHubLabels progress:(void (^)(MXBugReportState, NSProgress *))progress success:(void (^)(NSString*))success failure:(void (^)(NSError *))failure
{
    ...
    NSMutableURLRequest *request = [[AFHTTPRequestSerializer serializer] multipartFormRequestWithMethod:@"POST" URLString:apiPath parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        // Fill params defined in https://github.com/sending-network/rageshake#post-apisubmit
        if (text)
        {
...
#if __has_include(<SendingnetworkKit/SendingnetworkKit.h>)
        [formData appendPartWithFormData:[SendingnetworkKitVersion dataUsingEncoding:NSUTF8StringEncoding] name:@"sendingnetwork_kit_version"];
#endif

        [formData appendPartWithFormData:[SendingnetworkSDKVersion dataUsingEncoding:NSUTF8StringEncoding] name:@"sendingnetwork_sdk_version"];
