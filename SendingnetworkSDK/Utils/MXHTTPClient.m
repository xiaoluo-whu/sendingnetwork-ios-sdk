...
NSString* const MXHTTPClientErrorResponseDataKey = @"com.sendingnetworksdk.httpclient.error.response.data";
...
NSString* const kMXHTTPClientSendingnetworkErrorNotification = @"kMXHTTPClientSendingnetworkErrorNotification";
NSString* const kMXHTTPClientSendingnetworkErrorNotificationErrorKey = @"kMXHTTPClientSendingnetworkErrorNotificationErrorKey";
           ...
           failure:(void (^)(NSError *error))failure
{
    ...
    //FIXME: yk add acceptableContentTypes here maybe not suitable
    ...
    mxHTTPOperation.operation = [httpManager dataTaskWithRequest:request uploadProgress:^(NSProgress * _Nonnull theUploadProgress) {
        ...
        else
        {
            ...
            if (response)
            {
                // If the home server (or any other Sendingnetwork server) sent data, it may contain 'errcode' and 'error'.
                ...
                if (JSONResponse)
                {
                    ...
                    if (JSONResponse[kMXErrorCodeKey] || JSONResponse[kMXErrorMessageKey])
                    {
                        ...
                        // Send a notification
                        [[NSNotificationCenter defaultCenter] postNotificationName:kMXHTTPClientSendingnetworkErrorNotification
                                                                            object:self
                                                                          userInfo:@{ kMXHTTPClientSendingnetworkErrorNotificationErrorKey: mxError }];
