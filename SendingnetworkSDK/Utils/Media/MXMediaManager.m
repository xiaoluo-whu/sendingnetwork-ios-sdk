    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filePath])
    {
        ...
        if (imageContent)
        {
            ...
            if ([filePath hasSuffix:@"jpeg"] || [filePath hasSuffix:@"png"]) {
                ...
                SVGKImage *svgImage = [[SVGKImage alloc] initWithData:imageContent];
                res = svgImage.UIImage;
            }
...
+ (NSString*)downloadIdForSendingnetworkContentURI:(NSString*)mxContentURI
                                  inFolder:(NSString*)folder
{
    // Return the unique output file path built from the mxc uri and the potential folder (no type is required here)
    return [MXMediaManager cachePathForSendingnetworkContentURI:mxContentURI andType:nil inFolder:folder];
}

+ (NSString*)thumbnailDownloadIdForSendingnetworkContentURI:(NSString*)mxContentURI
                                           inFolder:(NSString*)folder
                                      toFitViewSize:(CGSize)viewSize
                                         withMethod:(MXThumbnailingMethod)thumbnailingMethod
{
    // Return the unique output file path built from the mxc uri and the potential folder (no type is required here)
    return [MXMediaManager thumbnailCachePathForSendingnetworkContentURI:mxContentURI andType:nil inFolder:folder toFitViewSize:viewSize withMethod:thumbnailingMethod];
}

- (MXMediaLoader*)downloadMediaFromSendingnetworkContentURI:(NSString *)mxContentURI
                                            ...
                                            failure:(void (^)(NSError *error))failure
{
    ...
    if (!mediaURL)
    {
        MXLogDebug(@"[MXMediaManager] downloadMediaFromSendingnetworkContentURI: invalid media content URI");
        if (failure) failure(nil);
        return nil;
    }
    
    // Build the outpout file path from mxContentURI, and other inputs.
    NSString *filePath = [MXMediaManager cachePathForSendingnetworkContentURI:mxContentURI andType:mimeType inFolder:folder];
    
    // Build the download id from mxContentURI.
    NSString *downloadId = [MXMediaManager downloadIdForSendingnetworkContentURI:mxContentURI inFolder:folder];
...
- (MXMediaLoader*)downloadMediaFromSendingnetworkContentURI:(NSString *)mxContentURI
                                           ...
                                           inFolder:(NSString *)folder
{
    return [self downloadMediaFromSendingnetworkContentURI:mxContentURI withType:mimeType inFolder:folder success:nil failure:nil];
}

- (MXMediaLoader*)downloadThumbnailFromSendingnetworkContentURI:(NSString *)mxContentURI
                                               withType:(NSString *)mimeType
                                               inFolder:(NSString *)folder
                                          toFitViewSize:(CGSize)viewSize
                                             withMethod:(MXThumbnailingMethod)thumbnailingMethod
                                                ...
                                                failure:(void (^)(NSError *error))failure
{
    ...
    if ([mimeType isEqualToString:@"svg"]) {
        mediaURL = mxContentURI;
    }else {
        // Check the provided mxc URI by resolving it into an HTTP URL.
        mediaURL = [self urlOfContentThumbnail:mxContentURI toFitViewSize:viewSize withMethod:thumbnailingMethod];
    }
   
    if (!mediaURL)
    {
        MXLogDebug(@"[MXMediaManager] downloadThumbnailFromSendingnetworkContentURI: invalid media content URI");
        if (failure) failure(nil);
        return nil;
    }
    
    // Build the outpout file path from mxContentURI, and other inputs.
    NSString *filePath = [MXMediaManager thumbnailCachePathForSendingnetworkContentURI:mxContentURI andType:mimeType inFolder:folder toFitViewSize:viewSize withMethod:thumbnailingMethod];
    
    // Build the download id from mxContentURI.
    NSString *downloadId = [MXMediaManager thumbnailDownloadIdForSendingnetworkContentURI:mxContentURI inFolder:folder toFitViewSize:viewSize withMethod:thumbnailingMethod];
                        ...
                        failure:(void (^)(NSError *error))failure
{
    ...
    if (mediaLoader)
    {
        // This mediaLoader has been created for the same sendingnetwork content uri, and cache folder.
        if (success || failure)
        {
...
- (MXMediaLoader*)downloadEncryptedMediaFromSendingnetworkContentFile:(MXEncryptedContentFile *)encryptedContentFile
                                                      ...
                                                      failure:(void (^)(NSError *error))failure
{
    ...
    if (!downloadMediaURL)
    {
        MXLogDebug(@"[MXMediaManager] downloadEncryptedMediaFromSendingnetworkContentFile: invalid media content URI");
        if (failure) failure(nil);
        return nil;
    }
    
    // Build the outpout file path from mxContentURI, and other inputs.
    NSString *filePath = [MXMediaManager cachePathForSendingnetworkContentURI:mxContentURI
                                                              andType:mimeType
                                                             inFolder:folder];
    
    // Build the download id from mxContentURI.
    NSString *downloadId = [MXMediaManager downloadIdForSendingnetworkContentURI:mxContentURI
                                                                inFolder:folder];
...
- (MXMediaLoader*)downloadEncryptedMediaFromSendingnetworkContentFile:(MXEncryptedContentFile *)encryptedContentFile
                                                     ...
                                                     inFolder:(NSString *)folder
{
    return [self downloadEncryptedMediaFromSendingnetworkContentFile:encryptedContentFile
                                                    mimeType:mimeType
                                                    inFolder:folder
                                                     success:nil
                                                     failure:nil];
...
+ (MXMediaLoader*)prepareUploaderWithSendingnetworkSession:(MXSession*)mxSession
                                           ...
                                           andRange:(CGFloat)range
{
    if (mxSession)
    {
        ...
        MXMediaLoader *mediaLoader = [[MXMediaLoader alloc] initForUploadWithSendingnetworkSession:mxSession initialRange:initialRange andRange:range];
        // Report this loader
        if (!uploadTableById)
        {
...
+ (NSString*)cachePathForSendingnetworkContentURI:(NSString*)mxContentURI andType:(NSString *)mimeType inFolder:(NSString*)folder
{
    ...
    {
        MXLogDebug(@"[MXMediaManager] cachePathForSendingnetworkContentURI: invalid media content URI");
        return nil;
    }
...
+ (NSString*)thumbnailCachePathForSendingnetworkContentURI:(NSString*)mxContentURI
...
//    if (![mxContentURI hasPrefix:kMXContentUriScheme])
//    {
//        MXLogDebug(@"[MXMediaManager] thumbnailCachePathForSendingnetworkContentURI: invalid media content URI");
//        return nil;
//    }
