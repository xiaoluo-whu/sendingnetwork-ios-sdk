...
- (instancetype)initWithIdentityServerRestClient:(MXIdentityServerRestClient*)identityServerRestClient andHomeserverRestClient:(MXRestClient*)homeserverRestClient
{
    ...
    if (self)
    {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleHTTPClientError:) name:kMXHTTPClientSendingnetworkErrorNotification object:nil];
...
- (void)handleHTTPClientError:(NSNotification*)nofitication
{
    ...
    MXError *mxError;
    MXJSONModelSet(mxError, [MXError class], nofitication.userInfo[kMXHTTPClientSendingnetworkErrorNotificationErrorKey]);
