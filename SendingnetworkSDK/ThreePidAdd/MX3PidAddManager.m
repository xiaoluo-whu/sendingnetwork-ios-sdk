...
NSString *const MX3PidAddManagerErrorDomain = @"org.sendingnetwork.sdk.MX3PidAddManagerErrorDomain";
...
- (instancetype)initWithSendingnetworkSession:(MXSession *)session
{
    self = [super init];
    if (self)
    {
                                                    ...
                                                    failure:(void (^)(NSError * _Nonnull))failure
{
    ...
    return [self->mxSession.sendingnetworkRestClient add3PIDOnlyWithSessionId:@"" clientSecret:[MXTools generateSecret] authParams:nil success:^{
                                           ...
                                           failure:(void (^)(NSError * _Nonnull))failure
{
    ...
    threePidAddSession.httpOperation = [self checkIdentityServerRequirementForAdding3PidWithSuccess:^{
        ...
        MXHTTPOperation *operation = [self->mxSession.sendingnetworkRestClient requestTokenForEmail:email isDuringRegistration:NO clientSecret:threePidAddSession.clientSecret sendAttempt:threePidAddSession.sendAttempt++ nextLink:nextLink success:^(NSString *sid) {
                           ...
                           failure:(void (^)(NSError * _Nonnull))failure
{
    ...
    if (doesServerSupportSeparateAddAndBind)
    {
        ...
        threePidAddSession.httpOperation = [mxSession.sendingnetworkRestClient add3PIDOnlyWithSessionId:threePidAddSession.sid clientSecret:threePidAddSession.clientSecret authParams:authParams success:^{
    ...
    else
    {
        ...
        threePidAddSession.httpOperation = [mxSession.sendingnetworkRestClient add3PID:threePidAddSession.sid clientSecret:threePidAddSession.clientSecret bind:NO success:^{
            threePidAddSession.httpOperation = nil;
                              ...
                              failure:(void (^)(NSError * _Nonnull))failure
{
    ...
    threePidAddSession.httpOperation = [self submitValidationToken:token for3PidAddSession:threePidAddSession success:^{
        ...
        {
            // https://gist.github.com/jryans/839a09bf0c5a70e2f36ed990d50ed928#2b-adding-a-3pid-to-hs-account-after-registration-post-msc2290
            operation = [self->mxSession.sendingnetworkRestClient add3PIDOnlyWithSessionId:threePidAddSession.sid clientSecret:threePidAddSession.clientSecret authParams:authParams success:^{
        ...
        {
            // https://gist.github.com/jryans/839a09bf0c5a70e2f36ed990d50ed928#2a-adding-a-3pid-to-hs-account-after-registration-pre-msc2290
            operation = [self->mxSession.sendingnetworkRestClient add3PID:threePidAddSession.sid clientSecret:threePidAddSession.clientSecret bind:NO success:^{
...
{
    MXWeakify(self);
    return [mxSession.sendingnetworkRestClient supportedSendingnetworkVersions:^(MXSendingnetworkVersions *sendingnetworkVersions) {
        MXStrongifyAndReturnIfNil(self);

        MXLogDebug(@"[MX3PidAddManager] checkIdentityServerRequirement: %@", sendingnetworkVersions.doesServerRequireIdentityServerParam ? @"YES": @"NO");

        MXLogDebug(@"[MX3PidAddManager] doesServerSupportSeparateAddAndBind: %@", sendingnetworkVersions.doesServerSupportSeparateAddAndBind ? @"YES": @"NO");
        self->doesServerSupportSeparateAddAndBind = sendingnetworkVersions.doesServerSupportSeparateAddAndBind;

        if (sendingnetworkVersions.doesServerRequireIdentityServerParam
            && !self->mxSession.sendingnetworkRestClient.identityServer)
        {
            NSError *error = [NSError errorWithDomain:MX3PidAddManagerErrorDomain
                                                 ...
                                                 failure:(void (^)(NSError * _Nonnull))failure
{
    ...
    else
    {
        ...
        operation = [mxSession.identityService accessTokenWithSuccess:^(NSString * _Nullable accessToken) {
            ...
            if (accessToken)
            {
                MXHTTPOperation *operation2 = [self->mxSession.sendingnetworkRestClient supportedSendingnetworkVersions:^(MXSendingnetworkVersions *sendingnetworkVersions) {
...
- (NSDictionary*)authParamsFromError:(NSError*)error andPassword:(nullable NSString*)password
{
    ...
    if (urlResponse && urlResponse.statusCode == 401)
    {
        ...
        {
            NSString *userId = self->mxSession.sendingnetworkRestClient.credentials.userId;
            authParams = @{
    ...
    threePidAddSession.httpOperation = [self doesServerSupportSeparateAddAndBind:^(BOOL doesServerSupportSeparateAddAndBind) {
        ...
        if (doesServerSupportSeparateAddAndBind)
        {
            ...
            else
            {
                ...
                operation = [self->mxSession.sendingnetworkRestClient unbind3PidWithAddress:threePidAddSession.address medium:threePidAddSession.medium success:^{
                                                      ...
                                                      failure:(void (^)(NSError * _Nonnull))failure
{
    ...
    if ([threePidAddSession.medium isEqualToString:kMX3PIDMediumEmail])
    {
        threePidAddSession.httpOperation = [mxSession.sendingnetworkRestClient bind3PidWithSessionId:threePidAddSession.sid clientSecret:threePidAddSession.clientSecret success:^{
    ...
    else if ([threePidAddSession.medium isEqualToString:kMX3PIDMediumMSISDN])
    {
        ...
        threePidAddSession.httpOperation = [self submitValidationToken:token for3PidAddSession:threePidAddSession success:^{
            ...
            MXHTTPOperation *operation = [self->mxSession.sendingnetworkRestClient bind3PidWithSessionId:threePidAddSession.sid clientSecret:threePidAddSession.clientSecret success:^{
                                                   ...
                                                   failure:(void (^)(NSError * _Nonnull))failure
{
    ...
    MXWeakify(self);
    MXHTTPOperation *operation;
    operation = [mxSession.sendingnetworkRestClient remove3PID:threePidAddSession.address medium:threePidAddSession.medium success:^{
        MXStrongifyAndReturnIfNil(self);

        MXHTTPOperation *operation2;
        if ([threePidAddSession.medium isEqualToString:kMX3PIDMediumEmail])
        {
            operation2 = [self->mxSession.sendingnetworkRestClient requestTokenForEmail:threePidAddSession.address isDuringRegistration:NO clientSecret:threePidAddSession.clientSecret sendAttempt:threePidAddSession.sendAttempt++ nextLink:nil success:^(NSString *sid) {
                                                      ...
                                                      failure:(void (^)(NSError * _Nonnull))failure
{
    ...
    if ([threePidAddSession.medium isEqualToString:kMX3PIDMediumEmail])
    {
        threePidAddSession.httpOperation = [mxSession.sendingnetworkRestClient add3PID:threePidAddSession.sid clientSecret:threePidAddSession.clientSecret bind:threePidAddSession.bind success:^{
            threePidAddSession.httpOperation = nil;
    ...
    else if ([threePidAddSession.medium isEqualToString:kMX3PIDMediumMSISDN])
    {
        ...
        threePidAddSession.httpOperation = [self submitValidationToken:token for3PidAddSession:threePidAddSession success:^{
            ...
            MXHTTPOperation *operation = [self->mxSession.sendingnetworkRestClient add3PID:threePidAddSession.sid clientSecret:threePidAddSession.clientSecret bind:threePidAddSession.bind success:^{
