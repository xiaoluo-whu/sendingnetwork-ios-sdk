...
@synthesize hasMetionMe = _hasMetionMe;
...
- (instancetype)initWithRoomId:(NSString *)roomId andSendingnetworkSession:(MXSession *)mxSession
{
    // Let's use the default store
    return [self initWithRoomId:roomId sendingnetworkSession:mxSession andStore:mxSession.store];
}

- (instancetype)initWithRoomId:(NSString *)roomId sendingnetworkSession:(MXSession *)mxSession andStore:(id<MXStore>)theStore
{
    self = [self init];
    if (self)
    {
        ...
        [self setSendingnetworkSession:mxSession];
        [self commonInit];
    }
...
- (void)setSendingnetworkSession:(MXSession *)mxSession
{
    if (!_mxSession)
    {
...
- (void)resetRoomStateData
{
    ...
    [room state:^(MXRoomState *roomState) {
        ...
        if (self.displayname == nil || self.avatar == nil)
        {
            ...
            // we reapply the Sendingnetwork name/avatar calculation algorithm.
...
- (BOOL)updateLocalUnreadEventCount
{
    ...
    // updateLocalUnreadMentionMeEventCount
    if (localUnreadEventCount >0) {
        NSUInteger localUnreadMetionEventMeCount = [self.mxSession.store localUnreadMentionMeEventCount:self.roomId
                                                                                          threadId:nil
                                                                                        withTypeIn:self.mxSession.unreadEventTypes];
        if (self.localUnreadMetionMeEventCount != localUnreadMetionEventMeCount) {
            self.localUnreadMetionMeEventCount = localUnreadMetionEventMeCount;
        }
    } else {
        _localUnreadMetionMeEventCount = 0;
    }
...
- (BOOL)hasMetionMe {
    return _localUnreadMetionMeEventCount > 0 ;
}
...
- (void)handleJoinedRoomSync:(MXRoomSync*)roomSync onComplete:(void (^)(void))onComplete
{
    ...
    [self.room state:^(MXRoomState *roomState) {
        ...
        if (roomSync.unreadNotifications)
        {
            ...
            // (see for example this issue https://github.com/sending-network/synapse/issues/2193).
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self)
    {
        ...
        _localUnreadMetionMeEventCount = (NSUInteger)[aDecoder decodeIntegerForKey:@"localUnreadMetionMeEventCount"];
...
- (void)encodeWithCoder:(NSCoder *)aCoder
{
    ...
    [aCoder encodeInteger:(NSInteger)_localUnreadMetionMeEventCount forKey:@"localUnreadMetionMeEventCount"];
...
- (NSUInteger)hash
{
    ...
    result = prime * result + _localUnreadMetionMeEventCount;
