...
- (instancetype)initWithRoom:(MXRoom*)theRoom andInitialEventId:(NSString*)initialEventId
{
    ...
    if (initialEventId)
    {
        ...
        [memoryStore openWithCredentials:theRoom.mxSession.sendingnetworkRestClient.credentials onComplete:nil failure:nil];
...
- (instancetype)initWithRoom:(MXRoom*)theRoom initialEventId:(NSString*)initialEventId andStore:(id<MXStore>)theStore
{
    if (self = [self init])
    {
        ...
        _state = [[MXRoomState alloc] initWithRoomId:room.roomId andSendingnetworkSession:room.mxSession andDirection:YES];
...
- (BOOL)canPaginate:(MXTimelineDirection)direction
{
    ...
    else
    {
        if (_isLiveTimeline)
        {
            // Sendingnetwork is not yet able to guess the future
            canPaginate = NO;
        }
        else
        {
...
- (MXHTTPOperation *)resetPaginationAroundInitialEventWithLimit:(NSUInteger)limit success:(void (^)(void))success failure:(void (^)(NSError *))failure
{
    ...
    MXWeakify(self);
    return [room.mxSession.sendingnetworkRestClient contextOfEvent:_initialEventId inRoom:room.roomId limit:limit filter:_roomEventFilter success:^(MXEventContext *eventContext) {
...
- (MXHTTPOperation *)paginate:(NSUInteger)numItems direction:(MXTimelineDirection)direction onlyFromStore:(BOOL)onlyFromStore complete:(void (^)(void))complete failure:(void (^)(NSError *))failure
{
    ...
    [self paginateFromStore:numItems direction:direction onComplete:^(NSArray<MXEvent *> *eventsFromStore) {
        ...
        MXWeakify(self);
        MXHTTPOperation *operation2 = [self->room.mxSession.sendingnetworkRestClient messagesForRoom:self.state.roomId from:paginationToken direction:direction limit:remainingNumItems filter:self.roomEventFilter success:^(MXPaginationResponse *paginatedResponse) {
            MXStrongifyAndReturnIfNil(self);
...
- (void)handlePaginationResponse:(MXPaginationResponse*)paginatedResponse direction:(MXTimelineDirection)direction onComplete:(void (^)(void))onComplete
{
    ...
    // End token might be ommited when end of the timeline is reached: https://github.com/sending-network/synapse/pull/12903
