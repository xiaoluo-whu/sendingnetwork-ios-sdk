...
@interface MXFileStore ()
{
    ...
    // Flag to indicate Sendingnetwork filters need to be stored
    ...
    dispatch_async(dispatchQueue, ^(void){
        ...
        @autoreleasepool
        {
            ...
            if (self->metaData && kMXFileVersion != self->metaData.version)
            {
                ...
                {
                    MXLogDebug(@"[MXFileStore] Sendingnetwork SDK until the version of 35 of MXFileStore caches all NSURLRequests unnecessarily. Clear NSURLCache");
...
- (MXSendingnetworkVersions *)supportedSendingnetworkVersions
{
    return metaData.supportedSendingnetworkVersions;
}

- (void)storeSupportedSendingnetworkVersions:(MXSendingnetworkVersions *)supportedSendingnetworkVersions
{
    if (metaData)
    {
        metaData.supportedSendingnetworkVersions = supportedSendingnetworkVersions;
        metaDataHasChanged = YES;
    }
}
...
#pragma mark - Sendingnetwork groups
/**
 Preload all groups.
