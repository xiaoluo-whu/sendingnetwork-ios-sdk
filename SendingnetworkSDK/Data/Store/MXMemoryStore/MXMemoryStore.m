...
@synthesize supportedSendingnetworkVersions;
...
- (NSUInteger)localUnreadMentionMeEventCount:(NSString*)roomId threadId:(NSString *)threadId withTypeIn:(NSArray*)types
{
    NSArray<MXEvent*> *newEvents = [self newIncomingEventsInRoom:roomId threadId:threadId withTypeIn:types];
    __block NSUInteger result = 0;
    // Check whether these unread events have not been redacted.
    ...
    {
        // TODO: coral
        if (!event.isRedactedEvent && [event containsMentionUser:credentials.userId])
        {
            //event.wireContent.
            result++;
        }
...
{
    supportedSendingnetworkVersions = supportedSendingnetworkVersions;
}
