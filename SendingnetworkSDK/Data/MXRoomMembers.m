...
- (instancetype)initWithRoomState:(MXRoomState *)roomState andSendingnetworkSession:(MXSession*)sendingnetworkSession
{
    self = [super init];
    if (self)
    {
        mxSession = sendingnetworkSession;
...
- (NSString*)memberName:(NSString*)userId
{
    ...
    {
        displayName = [self formatUserId:member.displayname];
    }
    ...
    if (displayName)
    {
        ...
        if (memberNameCount && [memberNameCount unsignedIntegerValue] > 1)
        {
            ...
            displayName = [NSString stringWithFormat:@"%@ (%@)", displayName, [self formatUserId:userId]];
        }
    }
    else
    {
        // By default, use the user ID
        displayName = [self formatUserId:userId];
    }
            ...
            switch (event.eventType)
            {
                case MXEventTypeRoomMember:
                {
                    ...
                    if (isLive && !event.isRedactedEvent && roomMember.membership == MXMembershipJoin)
                    {
                        ...
                        [user updateWithRoomMemberEvent:event roomMember:roomMember inSendingnetworkSession:mxSession];
...
- (NSString *)formatUserId:(NSString *)userId {
    ...
    NSString *contactId = array.firstObject;
    if ([contactId hasPrefix:@"@"]) {
        contactId = [contactId substringFromIndex:1];
    }
    NSString *host = @"";
    if (array.count >= 2) {
        host = array.lastObject;
    }
    return [NSString stringWithFormat:@"%@...%@:%@",[contactId substringToIndex:5],[contactId substringFromIndex:contactId.length - 4],host];
}
@end
