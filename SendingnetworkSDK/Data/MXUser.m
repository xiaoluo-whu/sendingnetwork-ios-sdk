...
- (void)updateWithRoomMemberEvent:(MXEvent*)roomMemberEvent roomMember:(MXRoomMember *)roomMember inSendingnetworkSession:(MXSession *)mxSession
{
...
- (void)updateWithPresenceEvent:(MXEvent*)presenceEvent inSendingnetworkSession:(MXSession *)mxSession
{
    ...
    // Note: It is about to change in a short future in Sendingnetwork spec.
...
- (void)updateFromHomeserverOfSendingnetworkSession:(MXSession *)mxSession success:(void (^)(void))success failure:(void (^)(NSError *))failure
{
    MXWeakify(self);
    [mxSession.sendingnetworkRestClient displayNameForUser:_userId success:^(NSString *displayname) {
        MXStrongifyAndReturnIfNil(self);

        MXWeakify(self);
        [mxSession.sendingnetworkRestClient avatarUrlForUser:self.userId success:^(NSString *avatarUrl) {
            MXStrongifyAndReturnIfNil(self);
            ...
            MXLogDebug(@"[MXUser] updateFromHomeserverOfSendingnetworkSession failed to get avatar");
            failure(error);
        }];
    } failure:^(NSError *error) {

        MXLogDebug(@"[MXUser] updateFromHomeserverOfSendingnetworkSession failed to get display name");
        failure(error);
    }];
}
