...
- (id)initWithRoomId:(NSString*)roomId
    andSendingnetworkSession:(MXSession*)sendingnetworkSession
        andDirection:(BOOL)isLive
{
    ...
    if (self)
    {
        ...
        _members = [[MXRoomMembers alloc] initWithRoomState:self andSendingnetworkSession:mxSession];
...
- (id)initWithRoomId:(NSString*)roomId
    andSendingnetworkSession:(MXSession*)sendingnetworkSession
        ...
        andDirection:(BOOL)isLive
{
    self = [self initWithRoomId:roomId andSendingnetworkSession:sendingnetworkSession andDirection:isLive];
    if (self)
    {
...
+ (void)loadRoomStateFromStore:(id<MXStore>)store
               ...
               sendingnetworkSession:(MXSession *)sendingnetworkSession
                  onComplete:(void (^)(MXRoomState *roomState))onComplete
{
    ...
    MXRoomState *roomState = [[MXRoomState alloc] initWithRoomId:roomId andSendingnetworkSession:sendingnetworkSession andDirection:YES];
    if (roomState)
    {
        [store stateOfRoom:roomId success:^(NSArray<MXEvent *> * _Nonnull stateEvents) {
            if (!stateEvents.count) {
                ...
                [sendingnetworkSession.sendingnetworkRestClient stateOfRoom:roomId success:^(NSArray *JSONData) {
