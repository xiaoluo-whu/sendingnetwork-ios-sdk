...
internal class MXStoreRoomListDataFetcher: NSObject, MXRoomListDataFetcher {
    ...
    ///func fetchRooms(contactIds: [String]?)
    @objc
    public func fetchContactList(contactIds:[String]?) {
        if fetchOptions.async {
            executionQueue.async { [weak self] in
                guard let self = self else { return }
                self.fetchRooms(contactIds: contactIds)
            }
        } else {
            executionQueue.sync { [weak self] in
                guard let self = self else { return }
    ...
    /// Compute data up to  all items
    @objc
    private func computeAllData() -> MXRoomListData {
        var rooms = Array(roomSummaries.values)
        rooms = filterRooms(rooms)
        rooms = sortRooms(rooms)
//        let totalRoomsCount = rooms.count
    ...
    private func fetchRooms(contactIds: [String]?) {
        let contactIdsTmp:[String] = contactIds ?? []
        
        self.roomSummaries.removeAll()
        for contactId in contactIdsTmp{
            for roomId in store.rooms {
                if roomId == contactId, let summary = store.summary(ofRoom: roomId) {
                    self.roomSummaries[roomId] = summary
                }
            }
        }
