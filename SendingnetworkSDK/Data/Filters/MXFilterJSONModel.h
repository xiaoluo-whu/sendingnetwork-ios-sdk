...
/**
 `MXFilterJSONModel` represents a sendingnetwork filter model as exchanged through the Sendingnetwork
 Client-Server API.
 */
...
/**
 Build a Sendingnetwork filter to retrieve a max given number of messages per room in /sync requests.

 @param messageLimit messageLimit the messages count limit.
 @return the Sendingnetwork filter.
 */
+ (MXFilterJSONModel*)syncFilterWithMessageLimit:(NSUInteger)messageLimit;

/**
 Build a Sendingnetwork filter to enable room members lazy loading in /sync requests.
