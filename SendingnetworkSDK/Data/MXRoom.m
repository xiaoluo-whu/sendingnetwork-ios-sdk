...
#import "MXSession.h"
#import "MXTools.h"
#import "NSData+SendingnetworkSDK.h"
...
- (id)initWithRoomId:(NSString *)roomId andSendingnetworkSession:(MXSession *)mxSession2
{
    // Let's the live MXEventTimeline use its default store.
    return [self initWithRoomId:roomId sendingnetworkSession:mxSession2 andStore:nil];
}

- (id)initWithRoomId:(NSString *)roomId sendingnetworkSession:(MXSession *)mxSession2 andStore:(id<MXStore>)store
{
    self = [self init];
    if (self)
    {
...
+ (id)loadRoomFromStore:(id<MXStore>)store withRoomId:(NSString *)roomId sendingnetworkSession:(MXSession *)sendingnetworkSession
{
    MXRoom *room = [[MXRoom alloc] initWithRoomId:roomId andSendingnetworkSession:sendingnetworkSession];
    if (room)
    {
    ...
    if (needToLoadLiveTimeline || pendingLiveTimelineRequesters)
    {
        ...
        if (!pendingLiveTimelineRequesters)
        {
            ...
            [mxSession.store loadRoomMessagesForRoom:self.roomId completion:^{
                ...
                MXWeakify(self);
                [MXRoomState loadRoomStateFromStore:self.mxSession.store withRoomId:self.roomId sendingnetworkSession:self.mxSession onComplete:^(MXRoomState *roomState) {
                    MXStrongifyAndReturnIfNil(self);
    ...
    [self liveTimeline:^(id<MXEventTimeline> liveTimeline) {
        ...
        else
        {
            ...
            if (!self->pendingMembersRequesters)
            {
                ...
                MXWeakify(self);
                MXHTTPOperation *operation2 = [self.mxSession.sendingnetworkRestClient membersOfRoom:self.roomId
                             ...
                             failure:(void (^)(NSError *error))failure
{
    ...
    return [mxSession.sendingnetworkRestClient sendEventToRoom:self.roomId threadId:threadId eventType:eventTypeString content:newContent txnId:txnId success:^(NSString *eventId) {
...
- (MXHTTPOperation*)sendStateEventOfType:(MXEventTypeString)eventTypeString
                                ...
                                stateKey:(NSString *)stateKey
                                 ...
                                 failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient sendStateEventToRoom:self.roomId eventType:eventTypeString content:content stateKey:stateKey success:success failure:failure];
}
                      ...
                      failure:(void (^)(NSError *error))failure
{
    ...
    MXMediaLoader *uploader = [MXMediaManager prepareUploaderWithSendingnetworkSession:mxSession initialRange:0 andRange:endRange];
    NSString *fakeMediaURI = uploader.uploadId;
    
    NSString *cacheFilePath = [MXMediaManager cachePathForSendingnetworkContentURI:fakeMediaURI andType:mimetype inFolder:self.roomId];
    ...
    roomOperation = [self preserveOperationOrder:event block:^{
        ...
        if (self.mxSession.crypto && [self.mxSession.crypto isRoomEncrypted:self.summary.roomId])
        {
            ...
            [MXEncryptedAttachments encryptAttachment:uploader localUrl:localURL success:^(MXEncryptedContentFile *result) {
                ...
                else
                {
                    ...
                    MXMediaLoader *thumbUploader = [MXMediaManager prepareUploaderWithSendingnetworkSession:self.mxSession initialRange:0.9 andRange:1];
        ...
        {
            // Launch the upload to the Sendingnetwork Content repository
            [uploader uploadData:imageData filename:filename mimeType:mimetype success:^(NSString *url) {
                ...
                NSString *actualCacheFilePath = [MXMediaManager cachePathForSendingnetworkContentURI:url andType:mimetype inFolder:self.roomId];
                NSError *error;
                           ...
                           failure:(void (^)(NSError *error))failure
{
    ...
    // directly from its cache
    // Pass this id in the URL is a nasty trick to retrieve it later
    MXMediaLoader *thumbUploader = [MXMediaManager prepareUploaderWithSendingnetworkSession:self.mxSession initialRange:0 andRange:0.1];
    NSString *fakeMediaURI = thumbUploader.uploadId;
    
    NSString *cacheFilePath = [MXMediaManager cachePathForSendingnetworkContentURI:fakeMediaURI andType:@"image/jpeg" inFolder:self.roomId];
        ...
        [MXTools convertVideoAssetToMP4:videoAsset
            ...
            if (self.mxSession.crypto && self.summary.isEncrypted)
            {
                [MXEncryptedAttachments encryptAttachment:thumbUploader data:videoThumbnailData success:^(MXEncryptedContentFile *result) {
                    ...
                    MXMediaLoader *videoUploader = [MXMediaManager prepareUploaderWithSendingnetworkSession:self.mxSession initialRange:0.1 andRange:1];
            ...
            else
            {
                ...
                [thumbUploader uploadData:videoThumbnailData filename:nil mimeType:@"image/jpeg" success:^(NSString *thumbnailUrl) {
                    ...
                    if (videoData)
                    {
                        ...
                        NSString *actualCacheFilePath = [MXMediaManager cachePathForSendingnetworkContentURI:thumbnailUrl andType:@"image/jpeg" inFolder:self.roomId];
                        NSError *error;
                        [[NSFileManager defaultManager] copyItemAtPath:cacheFilePath toPath:actualCacheFilePath error:&error];

                        MXMediaLoader *videoUploader = [MXMediaManager prepareUploaderWithSendingnetworkSession:self.mxSession initialRange:0.1 andRange:0.9];
                        ...
                        [videoUploader uploadData:videoData filename:filename mimeType:mimetype success:^(NSString *videoUrl) {
                            ...
                            NSString *actualCacheFilePath = [MXMediaManager cachePathForSendingnetworkContentURI:videoUrl andType:mimetype inFolder:self.roomId];
                            ...
                            // And send the Sendingnetwork room message video event to the homeserver (the sent state of the local echo will be updated, its local storage too).
                            MXHTTPOperation *operation2 = [self sendMessageWithContent:msgContent threadId:threadId localEcho:&event success:onSuccess failure:onFailure];
           ...
           keepActualFilename:(BOOL)keepActualName
{
    ...
    // directly from its cache
    // Pass this id in the URL is a nasty trick to retrieve it later
    MXMediaLoader *uploader = [MXMediaManager prepareUploaderWithSendingnetworkSession:self.mxSession initialRange:0 andRange:1];
    NSString *fakeMediaURI = uploader.uploadId;
...
- (MXHTTPOperation*)setTopic:(NSString*)topic
                     ...
                     failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient setRoomTopic:self.roomId topic:topic success:success failure:failure];
}
...
- (MXHTTPOperation*)setAvatar:(NSString*)avatar
                     ...
                     failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient setRoomAvatar:self.roomId avatar:avatar success:success failure:failure];
}
...
- (MXHTTPOperation*)setName:(NSString*)name
                    ...
                    failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient setRoomName:self.roomId name:name success:success failure:failure];
}
...
- (MXHTTPOperation *)setHistoryVisibility:(MXRoomHistoryVisibility)historyVisibility
                                  ...
                                  failure:(void (^)(NSError *))failure
{
    return [mxSession.sendingnetworkRestClient setRoomHistoryVisibility:self.roomId historyVisibility:historyVisibility success:success failure:failure];
}
...
- (MXHTTPOperation*)setJoinRule:(MXRoomJoinRule)joinRule
                        ...
                        failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient setRoomJoinRule:self.roomId joinRule:joinRule success:success failure:failure];
}
- (MXHTTPOperation*)setJoinRule:(MXRoomJoinRule)joinRule
                      parentIds:(NSArray<NSString *>*)parentIds
                        ...
                        failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient setRoomJoinRule:joinRule forRoomWithId:self.roomId allowedParentIds:parentIds success:success failure:failure];
}
...
- (MXHTTPOperation*)setGuestAccess:(MXRoomGuestAccess)guestAccess
                           ...
                           failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient setRoomGuestAccess:self.roomId guestAccess:guestAccess success:success failure:failure];
}
...
- (MXHTTPOperation*)setDirectoryVisibility:(MXRoomDirectoryVisibility)directoryVisibility
                                   ...
                                   failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient setRoomDirectoryVisibility:self.roomId directoryVisibility:directoryVisibility success:success failure:failure];
}
...
- (MXHTTPOperation*)addAlias:(NSString *)roomAlias
                     ...
                     failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient addRoomAlias:self.roomId alias:roomAlias success:success failure:failure];
}
...
- (MXHTTPOperation*)removeAlias:(NSString *)roomAlias
                     ...
                     failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient removeRoomAlias:roomAlias success:success failure:failure];
}
...
- (MXHTTPOperation*)setCanonicalAlias:(NSString *)canonicalAlias
                              ...
                              failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient setRoomCanonicalAlias:self.roomId canonicalAlias:canonicalAlias success:success failure:failure];
}

- (MXHTTPOperation*)directoryVisibility:(void (^)(MXRoomDirectoryVisibility directoryVisibility))success
                                failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient directoryVisibilityOfRoom:self.roomId success:success failure:failure];
}
                       ...
                       failure:(void (^)(NSError *error))failure
{
    ...
    return [mxSession.sendingnetworkRestClient inviteUser:userId toRoom:self.roomId success:success failure:failure];
}
...
- (MXHTTPOperation*)inviteUserByEmail:(NSString*)email
                              ...
                              failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient inviteUserByEmail:email toRoom:self.roomId success:success failure:failure];
}
...
- (MXHTTPOperation*)kickUser:(NSString*)userId
                     ...
                     failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient kickUser:userId fromRoom:self.roomId reason:reason success:success failure:failure];
}
...
- (MXHTTPOperation*)banUser:(NSString*)userId
                    ...
                    failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient banUser:userId inRoom:self.roomId reason:reason success:success failure:failure];
}
...
- (MXHTTPOperation*)unbanUser:(NSString*)userId
                      ...
                      failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient unbanUser:userId inRoom:self.roomId success:success failure:failure];
}
...
- (MXHTTPOperation*)sendTypingNotification:(BOOL)typing
                                   ...
                                   failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient sendTypingNotificationInRoom:self.roomId typing:typing timeout:timeout success:success failure:failure];
}
...
- (MXHTTPOperation*)redactEvent:(NSString*)eventId
                        ...
                        failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient redactEvent:eventId inRoom:self.roomId reason:reason success:success failure:failure];
}
...
- (MXHTTPOperation *)reportEvent:(NSString *)eventId
                         ...
                         failure:(void (^)(NSError *))failure
{
    return [mxSession.sendingnetworkRestClient reportEvent:eventId inRoom:self.roomId score:score reason:reason success:success failure:failure];
}
...
- (MXHTTPOperation*)setRelatedGroups:(NSArray<NSString *>*)relatedGroups
                        ...
                        failure:(void (^)(NSError *error))failure
{
    return [mxSession.sendingnetworkRestClient setRoomRelatedGroups:self.roomId relatedGroups:relatedGroups success:success failure:failure];
}
...
/**
 ...
 `> <@sender:sending.network> sent an image.\n\nReply message`
...
/**
 ...
 `<mx-reply><blockquote><a href=\"https://sendingnetwork.to/#/!vjFxDRtZSSdspfTSEr:sending.network/$15237084491191492ssFoA:sending.network\">In reply to</a> <a href=\"https://sendingnetwork.to/#/@sender:sending.network\">@sender:sending.network</a><br>sent an image.</blockquote></mx-reply>Reply message`
                   ...
                   failure:(void (^)(NSError *error))failure
{
    ...
    return [mxSession.sendingnetworkRestClient addTag:tag withOrder:order toRoom:self.roomId success:success failure:failure];
}
...
{
    // _accountData.tags will be updated by the live streams
    return [mxSession.sendingnetworkRestClient removeTag:tag fromRoom:self.roomId success:success failure:failure];
}
                     ...
                     failure:(void (^)(NSError *error))failure
{
    ...
    return [mxSession.sendingnetworkRestClient updateTaggedEvents:_roomId withContent:taggedEvents success:success failure:failure];
}
                       ...
                       failure:(void (^)(NSError *error))failure
{
    ...
    return [mxSession.sendingnetworkRestClient updateTaggedEvents:_roomId withContent:_accountData.taggedEvents success:success failure:failure];
}
...
- (MXHTTPOperation *)setAccountData:(NSDictionary *)content
                            ...
                            failure:(void (^)(NSError *))failure
{
    return [mxSession.sendingnetworkRestClient setRoomAccountData:_roomId
                                                eventType:type
...
- (void)acknowledgeEvent:(MXEvent*)event andUpdateReadMarker:(BOOL)updateReadMarker
{
    ...
    {
        [mxSession.sendingnetworkRestClient sendReadReceipt:self.roomId
...
- (void)setReadMarker:(NSString*)eventId withReadReceipt:(NSString*)receiptEventId
{
    ...
    [mxSession.sendingnetworkRestClient sendReadMarker:self.roomId readMarkerEventId:eventId readReceiptEventId:receiptEventId success:nil failure:nil];
}
...
#pragma mark - Favourite chats handling
...
- (MXHTTPOperation*)setIsContacter:(BOOL)isFavourite
                           success:(void (^)(void))success
                           failure:(void (^)(NSError *error))failure
{
    MXHTTPOperation *operation;
    ...
    operation = [self members:^(MXRoomMembers *roomMembers) {
        ...
        BOOL isRoom = YES;
        NSString *contactId = self.roomId;
        // is direct room
        if (members.count == 2) {
            isRoom = NO;
            for (MXRoomMember *member in members)
        ...
        if (isFavourite) {
            [self.mxSession.sendingnetworkRestClient addToFavoritesWithUserId:myUserId contactId:contactId isRoom:isRoom success:success failure:failure];
        }else {
            [self.mxSession.sendingnetworkRestClient deleteFavoritesWithUserId:myUserId contactId:contactId isRoom:isRoom success:success failure:failure];
        }
    } failure:failure];
