...
- (id)initWithRoomId:(NSString *)roomId andSendingnetworkSession:(MXSession *)mxSession
{
    ...
    [memoryStore openWithCredentials:mxSession.sendingnetworkRestClient.credentials onComplete:nil failure:nil];

    // Same thing for the summary
    summary = [[MXPeekingRoomSummary alloc] initWithRoomId:roomId sendingnetworkSession:mxSession andStore:memoryStore];
    [summary setPeekingRoom:self];

    return [self initWithRoomId:roomId sendingnetworkSession:mxSession andStore:memoryStore];
}
...
- (void)start:(void (^)(void))onServerSyncDone failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    httpOperation = [self.mxSession.sendingnetworkRestClient initialSyncOfRoom:self.roomId withLimit:0 success:^(MXRoomInitialSync *roomInitialSync) {
