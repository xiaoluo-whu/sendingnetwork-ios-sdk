...
public extension MXSession {
    ...
    */
    ...
    /// Sendingnetwork versions supported by the homeserver.
    /// 
    /// - Parameter completion: A block object called when the operation completes.
    /// - Returns: a `MXHTTPOperation` instance.
    @nonobjc @discardableResult func supportedSendingnetworkVersions(_ completion: @escaping (_ response: MXResponse<MXSendingnetworkVersions>) -> Void) -> MXHTTPOperation {
        return __supportedSendingnetworkVersions(currySuccess(completion), failure: curryFailure(completion))
    }
}
