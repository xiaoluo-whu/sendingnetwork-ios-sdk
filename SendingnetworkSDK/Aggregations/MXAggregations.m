...
- (MXHTTPOperation*)reactionsEventsForEvent:(NSString*)eventId
                                    ...
                                    failure:(void (^)(NSError *error))failure
{
    return [self.mxSession.sendingnetworkRestClient relationsForEvent:eventId
                                                       inRoom:roomId
                                    ...
                                    failure:(void (^)(NSError *error))failure
{
    ...
    if (!event)
    {
        operation = [self.mxSession.sendingnetworkRestClient relationsForEvent:eventId
                                                                inRoom:roomId
...
- (instancetype)initWithSendingnetworkSession:(MXSession *)mxSession
{
    ...
    if (self)
    {
        ...
        self.store = [[MXRealmAggregationsStore alloc] initWithCredentials:mxSession.sendingnetworkRestClient.credentials];

        self.aggregatedReactionsUpdater = [[MXAggregatedReactionsUpdater alloc] initWithSendingnetworkSession:self.mxSession aggregationStore:self.store];
        self.aggregatedEditsUpdater = [[MXAggregatedEditsUpdater alloc] initWithSendingnetworkSession:self.mxSession
                                                                             aggregationStore:self.store
                                                                                  sendingnetworkStore:mxSession.store];
        self.aggregatedReferencesUpdater = [[MXAggregatedReferencesUpdater alloc] initWithSendingnetworkSession:self.mxSession
                                                                                           sendingnetworkStore:mxSession.store];
