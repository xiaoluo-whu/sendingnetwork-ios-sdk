...
- (instancetype)initWithSendingnetworkSession:(MXSession *)mxSession aggregationStore:(id<MXAggregationsStore>)store
{
    self = [super init];
    if (self)
    {
        self.mxSession =mxSession;
        self.myUserId = mxSession.sendingnetworkRestClient.credentials.userId;
        self.store = store;
        self.sendingnetworkStore = mxSession.store;
...
- (void)handleReaction:(MXEvent *)event direction:(MXTimelineDirection)direction
{
    ...
    if (parentEventId && reaction)
    {
        ...
        MXEvent *parentEvent = [self.sendingnetworkStore eventWithEventId:parentEventId inRoom:event.roomId];
        if (parentEvent)
        {
...
- (void)updateReactionCountForReaction:(NSString*)reaction forEvent:(NSString*)eventId reactionEvent:(MXEvent *)reactionEvent
{
    ...
    // Migrate data from sendingnetwork store to aggregation store if needed
...
- (nullable NSArray<MXReactionCount*> *)reactionCountsOnEvent:(NSString*)eventId inRoom:(NSString*)roomId
{
    ...
    for (MXReactionRelation *relation in relations)
    {
        ...
        {
            // Have the same behavior as reactionCountsFromSendingnetworkStoreOnEvent
        ...
        if (!reactionCount.myUserReactionEventId)
        {
            ...
            MXEvent *event = [self.sendingnetworkStore eventWithEventId:relation.reactionEventId inRoom:roomId];
            if ([event.sender isEqualToString:self.myUserId])
            {
