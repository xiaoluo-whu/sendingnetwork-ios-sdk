...
- (void)handleReplace:(MXEvent *)replaceEvent
{
    ...
    MXEvent *event = [self.sendingnetworkStore eventWithEventId:replaceEvent.relatesTo.eventId inRoom:roomId];
    ...
    if (event)
    {
        ...
        if (![event.unsignedData.relations.replace.eventId isEqualToString:replaceEvent.eventId])
        {
            ...
            if (editedEvent)
            {
                [self.sendingnetworkStore replaceEvent:editedEvent inRoom:roomId];
