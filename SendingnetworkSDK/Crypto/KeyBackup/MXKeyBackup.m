...
- (void)sendKeyBackup
{
    ...
    // Make the request
    MXWeakify(self);
    [crypto.sendingnetworkRestClient sendKeysBackup:keysBackupData version:_keyBackupVersion.version success:^{
        MXStrongifyAndReturnIfNil(self);
...
{
    return [crypto.sendingnetworkRestClient keyBackupVersion:version success:success failure:^(NSError *error) {
                                    ...
                                    failure:(void (^)(NSError * _Nonnull))failure
{
    ...
    dispatch_async(cryptoQueue, ^{
        ...
        NSString *myUserId = self->crypto.sendingnetworkRestClient.credentials.userId;
                                   ...
                                   failure:(nullable void (^)(NSError *error))failure
{
    ...
    dispatch_async(cryptoQueue, ^{
        ...
        MXHTTPOperation *operation2 = [self->crypto.sendingnetworkRestClient createKeyBackupVersion:keyBackupVersion success:^(NSString *version) {
                                   ...
                                   failure:(nullable void (^)(NSError *error))failure
{
    ...
    dispatch_async(cryptoQueue, ^{
        ...
        MXWeakify(self);
        MXHTTPOperation *operation2 = [self->crypto.sendingnetworkRestClient deleteKeyBackupVersion:version success:^{
            MXStrongifyAndReturnIfNil(self);
...
{
    NSString *myUserId = crypto.sendingnetworkRestClient.credentials.userId;
                                   ...
                                   failure:(void (^)(NSError * _Nonnull))failure
{
    ...
    dispatch_async(cryptoQueue, ^{
        ...
        MXHTTPOperation *operation2 = [self->crypto.sendingnetworkRestClient updateKeyBackupVersion:newKeyBackupVersion success:^(void) {
                                ...
                                failure:(void (^)(NSError *error))failure;
{
    ...
    if (!sessionId && !roomId)
    {
        operation = [crypto.sendingnetworkRestClient keysBackup:version success:success failure:failure];
    }
    else if (!sessionId)
    {
        operation = [crypto.sendingnetworkRestClient keysBackupInRoom:roomId version:version success:^(MXRoomKeysBackupData *roomKeysBackupData) {
    ...
    {
        operation =  [crypto.sendingnetworkRestClient keyBackupForSession:sessionId inRoom:roomId version:version success:^(MXKeyBackupData *keyBackupData) {
...
- (MXKeyBackupData*)encryptGroupSession:(MXOlmInboundGroupSession*)session
{
    ...
    // https://github.com/uhoreg/sendingnetwork-doc/blob/e2e_backup/proposals/1219-storing-megolm-keys-serverside.md#mmegolm_backupv1curve25519-aes-sha2-key-format
