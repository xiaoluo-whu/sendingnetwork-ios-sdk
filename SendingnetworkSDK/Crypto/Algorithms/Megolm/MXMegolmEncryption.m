...

{
    ...
    operation = [crypto ensureOlmSessionsForDevices:devicesByUser force:NO success:^(MXUsersDevicesMap<MXOlmSessionResult *> *results) {
        ...
        if (haveTargets)
        {
            ...
            MXHTTPOperation *operation2 = [self->crypto.sendingnetworkRestClient sendToDevice:kMXEventTypeStringRoomEncrypted contentMap:contentMap txnId:nil success:^{
                       ...
                       failure:(void (^)(NSError *error))failure
{
    ...
    operation = [crypto ensureOlmSessionsForDevices:@{
                                            ...
                                            success:^(MXUsersDevicesMap<MXOlmSessionResult *> *results)
                 {
                     ...
                     MXHTTPOperation *operation2 = [self->crypto.sendingnetworkRestClient sendToDevice:kMXEventTypeStringRoomEncrypted contentMap:contentMap txnId:nil success:success failure:failure];
                     [operation mutateTo:operation2];
