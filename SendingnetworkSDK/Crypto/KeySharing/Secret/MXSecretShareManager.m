                        ...
                        failure:(void (^)(NSError *error))failure
{
    ...
    return [_crypto.sendingnetworkRestClient sendToDevice:kMXEventTypeStringSecretRequest contentMap:contentMap txnId:nil success:success failure:failure];
}
...
- (void)shareSecret:(NSString*)secret toRequest:(MXSecretShareRequest*)request
{
    ...
    [_crypto ensureOlmSessionsForDevices:userDevice force:NO success:^(MXUsersDevicesMap<MXOlmSessionResult *> *results) {
        ...
        [self.crypto.sendingnetworkRestClient sendToDevice:kMXEventTypeStringRoomEncrypted contentMap:contentMap txnId:nil success:nil failure:^(NSError *error) {
