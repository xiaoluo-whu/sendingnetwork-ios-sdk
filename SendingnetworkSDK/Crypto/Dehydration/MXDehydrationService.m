...
NSString *const MXDehydrationAlgorithm = @"org.sendingnetwork.msc2697.v1.olm.libolm_pickle";
NSString *const MXDehydrationServiceErrorDomain = @"org.sendingnetwork.sdk.dehydration.service";
...
- (void)dehydrateDeviceWithSendingnetworkRestClient:(MXRestClient*)restClient
                                    ...
                                    failure:(void (^)(NSError *error))failure;
{
    ...
    [restClient setDehydratedDevice:dehydratedDevice withDisplayName:@"Backup device" success:^(NSString *deviceId) {
        ...
        [crypto.crossSigning signObject:deviceInfo.signalableJSONDictionary withKeyType:MXCrossSigningKeyType.selfSigning success:^(NSDictionary *signedObject) {
            ...
            [self uploadDeviceInfo:deviceInfo forAccount:account withSendingnetworkRestClient:restClient success:success failure:failure];
...
- (void)rehydrateDeviceWithSendingnetworkRestClient:(MXRestClient*)restClient
                             dehydrationKey:(NSData*)dehydrationKey
                                    success:(void (^)(NSString * deviceId))success
                                    failure:(void (^)(NSError *error))failure;
{
...
- (void)uploadDeviceInfo:(MXDeviceInfo*)deviceInfo
    ...
    withSendingnetworkRestClient:(MXRestClient*)restClient
                 success:(void (^)(NSString *deviceId))success
                 failure:(void (^)(NSError *error))failure
{
