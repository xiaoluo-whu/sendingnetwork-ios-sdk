...
NSString *const MXCryptoMigrationErrorDomain = @"org.sendingnetwork.sdk.crypto.migration";
...
- (void)claimOwnOneTimeKeys:(NSUInteger)keyCount success:(void (^)(NSUInteger keyClaimed))success failure:(void (^)(NSError *))failure
{
    ...
    for (NSUInteger i = 0; i < keyCount; i++)
    {
        ...
        [crypto.sendingnetworkRestClient claimOneTimeKeysForUsersDevices:usersDevicesToClaim success:^(MXKeysClaimResponse *keysClaimResponse) {
