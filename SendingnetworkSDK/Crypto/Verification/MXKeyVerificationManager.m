...
NSString *const MXKeyVerificationErrorDomain = @"org.sendingnetwork.sdk.verification";
                     ...
                     failure:(void(^)(NSError *error))failure
{
    [self.crypto downloadKeys:@[userId] forceDownload:YES success:^(MXUsersDevicesMap<MXDeviceInfo *> *usersDevicesInfoMap, NSDictionary<NSString *,MXCrossSigningInfo *> *crossSigningKeysMap) {
        ...
        MXCredentials *myUser = self.crypto.mxSession.sendingnetworkRestClient.credentials;
                                         ...
                                         failure:(void(^)(NSError *error))failure
{
    ...
    [self.crypto.sendingnetworkRestClient sendToDevice:kMXMessageTypeKeyVerificationRequest contentMap:contentMap txnId:nil success:^{
...
- (instancetype)initWithCrypto:(MXCrypto *)crypto
{
    ...
    if (self)
    {
        ...
        statusResolver = [[MXKeyVerificationStatusResolver alloc] initWithManager:self sendingnetworkSession:crypto.mxSession];
...
- (void)handleReadyEvent:(MXEvent*)event isToDeviceEvent:(BOOL)isToDeviceEvent
{
    ...
    if (request)
    {
        MXCredentials *myCreds = _crypto.mxSession.sendingnetworkRestClient.credentials;
                          ...
                          failure:(void (^)(NSError *error))failure
{
    ...
    return [self.crypto.sendingnetworkRestClient sendToDevice:eventType contentMap:contentMap txnId:nil success:success failure:failure];
}
...
{
    NSString *currentUserId = self.crypto.mxSession.myUserId;
    NSString *currentDeviceId = self.crypto.mxSession.sendingnetworkRestClient.credentials.deviceId;
