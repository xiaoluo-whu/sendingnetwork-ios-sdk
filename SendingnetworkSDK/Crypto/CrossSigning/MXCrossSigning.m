...
NSString *const MXCrossSigningErrorDomain = @"org.sendingnetwork.sdk.crosssigning";
                  ...
                  failure:(void (^)(NSError *error))failure
{
    ...
    [self.crypto.sendingnetworkRestClient authSessionToUploadDeviceSigningKeys:^(MXAuthenticationSession *authSession) {
                    ...
                    failure:(void (^)(NSError *error))failure
{
    ...
    [self storeCrossSigningKeys:privateKeys success:^{
        ...
        [self.crypto.sendingnetworkRestClient uploadDeviceSigningKeys:signingKeys authParams:authParams success:^{
...
- (void)resetTrust
{
    MXLogDebug(@"[MXCrossSigning] resetTrust for device %@", self.crypto.mxSession.sendingnetworkRestClient.credentials.deviceId);
...
- (void)usersDevicesDidUpdate:(NSNotification*)notification
{
    ...
    MXCredentials *myUser = _crypto.mxSession.sendingnetworkRestClient.credentials;
           ...
           failure:(void (^)(NSError *error))failure
{
    ...
    [self signObject:object
             ...
             success:^(NSDictionary *signedObject)
     {
         ...
         [self.crypto.mxSession.sendingnetworkRestClient uploadKeySignatures:@{
                                                                       myUserId: @{
