...
#import "NSArray+SendingnetworkSDK.h"
...
+ (MXCrypto *)createCryptoWithSendingnetworkSession:(MXSession *)mxSession
{
    ...
    dispatch_queue_t cryptoQueue = [MXCrypto dispatchQueueForUser:mxSession.sendingnetworkRestClient.credentials.userId];
    dispatch_sync(cryptoQueue, ^{

        MXCryptoStoreClass *cryptoStore = [MXCryptoStoreClass createStoreWithCredentials:mxSession.sendingnetworkRestClient.credentials];
        cryptoStore.cryptoVersion = MXCryptoVersionLast;
        crypto = [[MXCrypto alloc] initWithSendingnetworkSession:mxSession cryptoQueue:cryptoQueue andStore:cryptoStore];
...
+ (void)checkCryptoWithSendingnetworkSession:(MXSession*)mxSession complete:(void (^)(MXCrypto *crypto))complete
{
#ifdef MX_CRYPTO
    ...
    MXLogDebug(@"[MXCrypto] checkCryptoWithSendingnetworkSession for %@", mxSession.sendingnetworkRestClient.credentials.userId);
    ...
    dispatch_async(cryptoQueue, ^{
        ...
        if ([MXCryptoStoreClass hasDataForCredentials:mxSession.sendingnetworkRestClient.credentials])
        {
            MXLogDebug(@"[MXCrypto] checkCryptoWithSendingnetworkSession: Crypto store exists");

            // If it already exists, open and init crypto
            MXCryptoStoreClass *cryptoStore = [[MXCryptoStoreClass alloc] initWithCredentials:mxSession.sendingnetworkRestClient.credentials];

            [cryptoStore open:^{

                MXLogDebug(@"[MXCrypto] checkCryptoWithSendingnetworkSession: Crypto store opened");

                MXCrypto *crypto = [[MXCrypto alloc] initWithSendingnetworkSession:mxSession cryptoQueue:cryptoQueue andStore:cryptoStore];
                ...
                MXLogDebug(@"[MXCrypto] checkCryptoWithSendingnetworkSession: Crypto store failed to open. Error: %@", error);
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                 ...
                 && mxSession.sendingnetworkRestClient.credentials.deviceId)
        {
            MXLogDebug(@"[MXCrypto] checkCryptoWithSendingnetworkSession: Need to create the store");
...
- (void)deleteStore:(void (^)(void))onComplete;
{
    ...
    dispatch_async(_cryptoQueue, ^{
        ...
        [MXCryptoStoreClass deleteStoreWithCredentials:self.mxSession.sendingnetworkRestClient.credentials];
      ...
      failure:(void (^)(NSError *error))failure
{
    ...
    startOperation = [self uploadDeviceKeys:^(MXKeysUploadResponse *keysUploadResponse) {
        ...
        // TODO: sendingnetwork-js-sdk does not do it anymore and waits for the completion
...
- (void)close:(BOOL)deleteStore
{
    ...
    dispatch_sync(_cryptoQueue, ^{
        ...
        [self.sendingnetworkRestClient close];
        self->_sendingnetworkRestClient = nil;
...
- (MXEventDecryptionResult *)decryptEvent:(MXEvent *)event inTimeline:(NSString*)timeline
{
    ...
    // TODO: dispatch_async (https://github.com/sending-network/sendingnetwork-ios-sdk/issues/205)
...
- (void)setUserVerification:(BOOL)verificationStatus forUser:(NSString*)userId
                    ...
                    failure:(void (^)(NSError *error))failure
{
    // We cannot remove cross-signing trust for a user in the sendingnetwork spec
...
- (instancetype)initWithSendingnetworkSession:(MXSession*)sendingnetworkSession cryptoQueue:(dispatch_queue_t)theCryptoQueue andStore:(id<MXCryptoStore>)store
{
    // This method must be called on the crypto thread
    self = [super init];
    if (self)
    {
        _mxSession = sendingnetworkSession;
        ...
        decryptionQueue = [MXCrypto dispatchQueueForUser:_mxSession.sendingnetworkRestClient.credentials.userId];
        ...
        _sendingnetworkRestClient = [[MXRestClient alloc] initWithCredentials:_mxSession.sendingnetworkRestClient.credentials andOnUnrecognizedCertificateBlock:nil andPersistentTokenDataHandler:_mxSession.sendingnetworkRestClient.persistTokenDataHandler andUnauthenticatedHandler:_mxSession.sendingnetworkRestClient.unauthenticatedHandler];
        _sendingnetworkRestClient.completionQueue = _cryptoQueue;
        ...
        NSString *userId = _sendingnetworkRestClient.credentials.userId;
        ...
        outgoingRoomKeyRequestManager = [[MXOutgoingRoomKeyRequestManager alloc]
                                         initWithSendingnetworkRestClient:_sendingnetworkRestClient
        ...
        _secretStorage = [[MXSecretStorage alloc] initWithSendingnetworkSession:_mxSession processingQueue:_cryptoQueue];
...

{
    ...
    return [_sendingnetworkRestClient claimOneTimeKeysForUsersDevices:usersDevicesToClaim success:^(MXKeysClaimResponse *keysClaimResponse) {
...
- (NSDictionary*)encryptMessage:(NSDictionary*)payloadFields forDevices:(NSArray<MXDeviceInfo*>*)devices
{
    ...
    for (MXDeviceInfo *recipientDevice in devices)
    {
        ...
        if (sessionId)
        {
            ...
            payloadJson[@"sender"] = _sendingnetworkRestClient.credentials.userId;
...
- (void)invalidateDeviceListsSince:(NSString*)oldSyncToken to:(NSString*)lastKnownSyncToken
                           ...
                           failure:(void (^)(NSError *error))failure
{
    [_sendingnetworkRestClient keyChangesFrom:oldSyncToken to:lastKnownSyncToken success:^(MXDeviceListResponse *deviceLists) {
...
{
    // Sanity check
    if (!_sendingnetworkRestClient.credentials.userId)
    {
        MXLogError(@"[MXCrypto] uploadDeviceKeys. ERROR: _sendingnetworkRestClient.credentials.userId cannot be nil");
        failure(nil);
        return nil;
    }
    ...
    _myDevice.signatures = @{
                            _sendingnetworkRestClient.credentials.userId: @{
    ...
    return [_sendingnetworkRestClient uploadKeys:_myDevice.JSONDictionary oneTimeKeys:nil fallbackKeys:nil success:success failure:failure];
}
    ...
    if ([self generateOneTimeKeys:keyCount])
    {
        operation = [self uploadOneTimeKeys:^(MXKeysUploadResponse *keysUploadResponse) {
            ...
            if ([MXError isMXError:error] && retry)
            {
                ...
                // There is no sendingnetwork specific error but we really want to detect the error described at
...
- (MXHTTPOperation *)generateAndUploadFallbackKey
{
    ...
    MXWeakify(self);
    return [_sendingnetworkRestClient uploadKeys:nil oneTimeKeys:nil fallbackKeys:fallbackKeyJson success:^(MXKeysUploadResponse *keysUploadResponse) {
        MXStrongifyAndReturnIfNil(self);
...
- (MXHTTPOperation *)uploadOneTimeKeys:(void (^)(MXKeysUploadResponse *keysUploadResponse))success failure:(void (^)(NSError *))failure
{
    ...
    // same one as used in login.
    MXWeakify(self);
    return [_sendingnetworkRestClient uploadKeys:nil oneTimeKeys:oneTimeJson fallbackKeys:nil success:^(MXKeysUploadResponse *keysUploadResponse) {
        MXStrongifyAndReturnIfNil(self);
...
{
    return [_sendingnetworkRestClient uploadKeys:_myDevice.JSONDictionary oneTimeKeys:nil fallbackKeys:nil success:^(MXKeysUploadResponse *keysUploadResponse) {
...
- (void)markOlmSessionForUnwedgingInEvent:(MXEvent*)event
{
    ...
    [self ensureOlmSessionsForDevices:userDevice force:YES success:^(MXUsersDevicesMap<MXOlmSessionResult *> *results) {
        ...
        [self.sendingnetworkRestClient sendToDevice:kMXEventTypeStringRoomEncrypted contentMap:contentMap txnId:nil success:nil failure:^(NSError *error) {
