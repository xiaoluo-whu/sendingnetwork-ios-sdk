...
extension MXThreadingServiceError: CustomNSError {
    public static let errorDomain = "org.sendingnetwork.sdk.threadingservice"
...
public class MXThreadingService: NSObject {
    ...
    public func allThreads(inRoom roomId: String,
        ...
        dispatchGroup.enter()
        let operation = session.supportedSendingnetworkVersions { response in
            switch response {
        ...
        dispatchGroup.notify(queue: .main) {
            if serverSupportThreads {
                ...
                let newOperation = session.sendingnetworkRestClient.messages(forRoom: roomId,
                                                                     from: "",
                                                                     direction: .backwards,
                                                                     limit: nil,
    ...
    private func thread(forRootEvent rootEvent: MXEvent, session: MXSession) -> MXThreadModel {
        ...
        //  workaround for https://github.com/sending-network/synapse/issues/11753. Can be removed when that's fixed.
