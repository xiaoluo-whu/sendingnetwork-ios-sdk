...
It automatically downloads the symbols file from the Sendingnetwork Jenkins and thus
...
def symbolicate(crashLogData):
    ...
    if not os.path.exists(dSYMSFile):
        ...
        # No, get it from the Sendingnetwork Jenkins
        if app == "sendingnetworkConsole":
             if "develop" in buildVersion:
                  jenkinsJob = "SendingnetworkConsoleiOSDevelop"
             else:
                  jenkinsJob = "SendingnetworkConsoleiOS"
        elif app == "Riot":
             if "develop" in buildVersion:
        ...
        XCArchiveTarGzURL = "http://sending.network/jenkins/view/SendingnetworkView/job/%s/%s/artifact/out/*.tar.gz/*zip*/out.zip" % (jenkinsJob, jenkinsBuild)
